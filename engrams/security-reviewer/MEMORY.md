...
- **MED**: `~/scripts/opencode-queue.py` — executes tasks from `~/notes/agent-queue.yaml` without validating `backend` or `working_dir`. YAML file is in the Obsidian vault (synced, pull-on-session).
...
4. **YAML/config files in synced vault as untrusted input to command execution** — agent-queue.yaml is in `~/notes/` which is Obsidian-synced and git-pulled on every session. Validate fields from this source.
