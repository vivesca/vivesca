...
**MCP over CLI only when state or schema justifies it.** CLIs with skills are the default. Wrap in MCP ONLY when one of these holds: (a) persistent state across calls (trogocytosis browser), (b) complex typed schemas LLMs mis-parse as CLI args (transposase multi-step workflows), (c) cross-client distribution matters (install once, use from CC/Gemini/Codex). For stateless single-param lookups (lysin-style), CLI + skill beats MCP wrapper. Don't auto-wrap every CLI in MCP — that's theater, not architecture.
