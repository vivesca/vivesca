...
**Architect-implementer split.** CC designs and reviews; droid/Goose implements. Both have full organism access (MCP, genome, memory, skills). The scarcity is model quality — Claude for judgment, GLM for unlimited coding tokens. **CC must not write implementation code.** Instead: write a sortase plan file with exact specs, dispatch to droid (`sortase exec <plan> -p <dir> -b droid`), review the output. Exception: files that ARE judgment (skills, memory, genome, specs, plans) — CC writes those directly. For batch work, use `/centrosome`.
