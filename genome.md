...
The organism's facts. CC is cytosol — runtime, internal — and the LLM is symbiont, external and probabilistic. Their trajectories run opposite: cytosol gets stronger over time, while the symbiont surface gets smaller. Move logic from symbiont to cytosol whenever it can be made deterministic.
...
Interaction conventions follow. Gather before responding. When Terry reports something, collect all available data first, then synthesise in one response. The first response should already contain the full picture.
