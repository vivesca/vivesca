...
**CLI + skill by default; MCP only where it earns its keep.** CLIs with skills are the default — install once, works in every shell-capable agent (CC, Codex, Gemini CLI, Goose). Wrap in MCP ONLY when one of these holds: (a) persistent state across calls (trogocytosis browser, ribosome_dispatch queues), (b) complex typed multi-verb schemas LLMs mis-parse as CLI args (transposase scan|plan|execute|verify), (c) the target agent harness cannot spawn shell (hosted sandboxes without exec, web UIs, constrained environments). "Cross-client distribution" is NOT a valid MCP criterion when the clients have shell — CLIs on PATH are already cross-client for every shell-capable harness. For stateless single-param lookups (lysin-style), CLI + skill beats MCP wrapper every time. Don't auto-wrap every CLI in MCP — that's theater, not architecture. 2026-04 lesson from the CLI-vs-MCP debate (Willison Oct/Nov 2025, Cloudflare Code Mode Feb 2026): the pattern replacing 2025's "wrap everything in MCP" is progressive disclosure via filesystem + `--help`, and vivesca's ~75 MCP tools need auditing down to ~15 that pass these criteria.
