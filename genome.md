...
**CLI + skill by default; MCP only where it earns its keep.** CLIs with skills are the default — install once, works in every shell-capable agent (CC, Codex, Gemini CLI, Goose). Wrap in MCP ONLY when one of these holds: (a) persistent state across calls (trogocytosis browser, ribosome_dispatch queues), (b) complex typed multi-verb schemas LLMs mis-parse as CLI args (transposase scan|plan|execute|verify), (c) the target agent harness cannot spawn shell (hosted sandboxes without exec, web UIs, constrained environments), (d) the tool needs the agent's own LLM for judgment inside the tool (filtering, reranking, relevance decisions) — MCP's process boundary prevents this and forces you into a second model with a separate API key (Milvus/Zilliz lesson, Apr 2026). "Cross-client distribution" is NOT a valid MCP criterion when the clients have shell — CLIs on PATH are already cross-client for every shell-capable harness. **stdio vs HTTP matters:** MCP over stdio is the fragile local version (process isolation, env leaks, high overhead) that CLI + skill mostly replaces; MCP over HTTP still genuinely earns its keep for enterprise tooling with centralised OAuth, RBAC, audit trails, and telemetry — Kong, Pomerium, MintMCP, Strata, Aembit are the vendor category here. For stateless single-param lookups (lysin-style), CLI + skill beats MCP wrapper every time. Don't auto-wrap every CLI in MCP — that's theater, not architecture. **Always measure before migrating.** 2026-04-05 measurement: vivesca's actual 46-tool MCP surface is ~2,500 tokens total (~1.25% of 200K context), not the "tens of thousands" the published benchmarks warn about — see `finding_vivesca_mcp_context_cost.md`. The published pain points (GitHub MCP at 55K tokens, 3-server setups at 143K) don't describe vivesca. Audit scope is small not big: 9 heavy tools (≥300 char schemas, 51% of surface) are slim-in-place candidates; the 31 tiny tools are theater to migrate. **Judex applies:** measure your own system before scoping a migration from published benchmarks — the numbers on someone else's stack say nothing about yours. **The 4 criteria are a direction, not a sharp line.** Sharp at the extremes (stateless single-param = CLI always; persistent session = MCP always), directional for ~50% of tools, genuine judgment for the remaining ~20%. Empirical proxies beat abstract reasoning: token cost, LLM parse reliability, "needs agent LLM in-loop" are all behavioral not architectural. Real choice space is THREE-way (CLI + skill, skill alone using agent LLM natively, MCP) — the Zilliz four-layer framing (CLI / Skills / REST / MCP HTTP) is cleaner than a two-way framing when deciding per tool.
