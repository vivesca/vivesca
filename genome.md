...
**Residual fuzziness is at one boundary only:** step 3's "structured record" line (when does a 2-field parameter count as structured vs flat?). Heuristic: if the caller would naturally `json.dumps({...})` in Python to build the argument, it's structured; if they'd use `subprocess.run([..., "--from", a, "--to", b])`, it's flat. Everything else in the tree is binary. Don't invent fuzziness to feel safer — the tree is as sharp as the problem allows.
