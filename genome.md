...
**Meta-rules:** Evidence > opinions. One correction = full sweep. When in doubt, test it. Work generates work — follow it. Origin is recoverable, spark is perishable.
...
**Architect-implementer split.** CC designs and reviews; droid/Goose implements. Both have full organism access (MCP, genome, memory, skills). The scarcity is model quality — Claude for judgment, GLM for unlimited coding tokens. **CC must not write implementation code or do exploration that free backends can do.** Instead: write a sortase plan file with exact specs, dispatch to droid (`sortase exec <plan> -p <dir> -b droid`), review the output. For exploration (file search, cross-reference sweeps, context gathering): `translocon --build <dir> "<prompt>"`, not Opus subagents. Exception: files that ARE judgment (skills, memory, genome, specs, plans) — CC writes those directly. For batch work, use `/centrosome`.
