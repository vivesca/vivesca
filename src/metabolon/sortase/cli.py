...
@click.option(
    ...
    default=True,
    show_default=True,
    help="Auto-commit changes after successful execution.",
)
...
@click.option(
    "--retries", default=1, show_default=True, type=int, help="Retry failed tasks N times."
)
...
@click.option(
    ...
    default=True,
    show_default=True,
    help="Execute each task in an isolated git worktree.",
)
def exec_command(
    plan_file: Path,
    project_dir: Path,
    ...
    try:
        ...
        pre_test_count = len(
            [
                line
                for line in _pre_test.stdout.splitlines()
    ...
    if not json_out and not quiet:
        for result in results:
            ...
            suffix = (
                f" [yellow]({timeout_pct:.0f}% of timeout)[/yellow]" if timeout_pct > 80 else ""
            )
            console.print(
                f"{result.task_name}: {result.tool} [{status}] ({duration:.1f}s){suffix}"
            )
    ...
    duration_s = round(
        sum(attempt.duration_s for result in results for attempt in result.attempts), 3
    )
    entry = {
        ...
        "failure_reason": next(
            (issue.check for issue in validation_issues if issue.severity == "error"), None
        ),
        "files_changed": changed_files,
        "tests_passed": 0 if any(issue.check == "tests" for issue in validation_issues) else 1,
        "cost_estimate": summarize_cost_estimates(
            [r.cost_estimate for r in results if r.cost_estimate]
        ),
    }
    append_log(entry)
    if json_out:
    ...
    if pre_test_count is not None and not dry_run and not quiet:
        try:
            ...
            post_test_count = len(
                [
                    line
                    for line in _post_test.stdout.splitlines()
                    if line.strip() and not line.startswith("no tests")
                ]
            )
            ...
            if delta > 0:
                console.print(
                    f"[green]Tests: {pre_test_count} → {post_test_count} (+{delta})[/green]"
                )
            elif delta < 0:
...
def graph_exec_command(
    ...
    if result.get("status") == "paused":
        console.print(
            f"[bold yellow]Paused[/bold yellow] before execution (thread: {result['thread_id']})"
        )
        console.print("\nRouting decisions:")
...
@click.option(
    "--last", "last_n", default=10, show_default=True, type=int, help="Show last N entries."
)
...
@click.option(
    ...
    default=None,
    type=click.Path(path_type=Path),
    help="Path to log.jsonl (default: sortase default).",
)
@click.option(
    "--coaching",
    "coaching_path",
    default=None,
    type=click.Path(path_type=Path),
    help="Path to coaching notes file.",
)
...
def status(clean: bool) -> None:
    ...
    stale_entries = [
        entry for entry in entries if isinstance(entry, dict) and entry.get("alive") is False
    ]
    ...
    if clean and stale_entries:
        surviving = [
            ...
            if not (isinstance(entry, dict) and entry.get("alive") is False)
        ]
    ...
    for entry in entries:
        if not isinstance(entry, dict):
            continue
        status_label = (
            "[red]STALE[/red]" if entry.get("alive") is False else "[green]running[/green]"
        )
    ...
    if stale_entries and not clean:
        console.print(
            f"[yellow]Hint: run 'sortase status --clean' to remove {len(stale_entries)} stale entry(ies).[/yellow]"
        )
...
@click.option(
    ...
    default=None,
    type=click.Path(path_type=Path),
    help="Write results JSONL for morning review.",
)
def watch(
    watch_dir: Path,
    project_dir: Path,
    ...
    console.print(
        f"Watching {watch_dir} for plan files (interval={interval}s, timeout={timeout}s)"
    )
    if max_concurrent != 1:
    ...
    async def _execute_plan(plan_name: str, plan_path: Path) -> None:
        ...
        async with semaphore:
            ...
            console.print(
                f"  TASK: {plan_name} | RESULT: {result_label} | DURATION: {duration_s}s"
            )
...
@click.option(
    ...
    show_default=True,
    type=int,
    help="Number of recent dispatches to show.",
)
@click.option("--json-output", "json_out", is_flag=True, help="Output as JSON array.")
...
@click.option(
    "--output",
    "output_path",
    default=Path.home() / "tmp" / "sortase-dashboard.html",
    type=click.Path(path_type=Path),
...
def dashboard(output_path: Path, log_path: Path | None) -> None:
    ...
    for idx, date_key in enumerate(sorted_dates):
        ...
        ok_height = (
            int((bucket["ok"] / max_per_date) * (bar_chart_height - 30)) if max_per_date else 0
        )
        fail_height = (
            int((bucket["fail"] / max_per_date) * (bar_chart_height - 30)) if max_per_date else 0
        )
        y_ok = bar_chart_height - 20 - ok_height
        y_fail = y_ok - fail_height
        if ok_height > 0:
            bar_chart_svg_parts.append(
                f'<rect x="{x_base + 5}" y="{y_ok}" width="18" height="{ok_height}" fill="#4ade80" rx="2"><title>{date_key}: {bucket["ok"]} ok</title></rect>'
            )
        if fail_height > 0:
            bar_chart_svg_parts.append(
                f'<rect x="{x_base + 25}" y="{y_fail}" width="18" height="{fail_height}" fill="#f87171" rx="2"><title>{date_key}: {bucket["fail"]} fail</title></rect>'
            )
        label = date_key[5:]  # MM-DD
        bar_chart_svg_parts.append(
            f'<text x="{x_base + 24}" y="{bar_chart_height - 4}" text-anchor="middle" font-size="9" fill="#94a3b8">{label}</text>'
        )
    ...
    if failure_reasons:
        ...
        for color_idx, (reason, count) in enumerate(failure_reasons.most_common()):
            ...
            if slice_angle >= 359.9:
                pie_parts.append(
                    f'<circle cx="{pie_center_x}" cy="{pie_center_y}" r="{pie_radius}" fill="{color}"/>'
                )
            else:
                pie_parts.append(
            ...
            legend_parts.append(
                f'<div style="display:flex;align-items:center;gap:8px;margin:4px 0;"><span style="display:inline-block;width:14px;height:14px;border-radius:3px;background:{color};"></span><span style="color:#cbd5e1;">{reason} ({count}, {pct:.0f}%)</span></div>'
            )
    ...
    for idx, (backend_name, count) in enumerate(backend_counts.most_common()):
        ...
        backend_bar_parts.append(
            f'<text x="0" y="{y + 14}" font-size="12" fill="#cbd5e1">{backend_name}</text>'
        )
        backend_bar_parts.append(
            f'<rect x="80" y="{y}" width="{bar_width}" height="20" fill="{color}" rx="3"><title>{backend_name}: {count} runs</title></rect>'
        )
        backend_bar_parts.append(
            f'<text x="{85 + bar_width}" y="{y + 14}" font-size="11" fill="#94a3b8">{count}</text>'
        )
    ...
    if avg_duration_by_date:
        ...
        for idx, date_key in enumerate(trend_dates):
            ...
            y = trend_area_bottom - int(
                ((avg - min_dur) / dur_range) * (trend_area_bottom - trend_area_top - 10)
            )
            line_points.append(f"{x},{y}")
            dot_parts.append(
                f'<circle cx="{x}" cy="{y}" r="4" fill="#38bdf8"><title>{date_key}: {avg:.1f}s avg</title></circle>'
            )
            label = date_key[5:]
            dot_parts.append(
                f'<text x="{x}" y="{trend_chart_height - 6}" text-anchor="middle" font-size="9" fill="#94a3b8">{label}</text>'
            )

        polyline = (
            f'<polyline points="{" ".join(line_points)}" fill="none" stroke="#38bdf8" stroke-width="2"/>'
...
@click.option(
    "--summary", "show_summary", is_flag=True, help="Show only the summary, not the full diff."
)
...
def clean_command(project_dir: Path, dry_run: bool) -> None:
    ...
    console.print(
        f"sortase clean: found {total} stale worktree(s)"
        if not dry_run
        else f"sortase clean: would remove {total} stale worktree(s)"
    )
