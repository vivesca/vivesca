...
def register_running(
    task_name: str, tool: str | None = None, project_dir: Path | None = None
) -> None:
    ...
    try:
        for retry_index in range(max_retries + 1):
            if retry_index > 0:
                if verbose:
                    retry_label = (
                        f"[{task.name}] retry {retry_index}/{max_retries}: resetting git state"
                    )
...
def _merge_worktree(project_dir: Path, worktree_path: Path) -> tuple[bool, str]:
    ...
    if conflicted:
        ...
        return (
            False,
            f"conflict: files modified in both branches: {', '.join(conflicted)}. Branch '{branch}' preserved.",
        )
