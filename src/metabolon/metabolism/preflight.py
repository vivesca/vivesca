...
PROVIDER_CONFIG = {
    "zhipu": {
        ...
        "base_url": "https://open.bigmodel.cn/api/paas/v4/models",
    },
    "volcano": {
...
def check_repo_reachable(path: Path, name: str) -> CheckResult:
    ...
    if not git_dir.exists():
        return CheckResult(
            name=f"{name}_reachable",
            passed=False,
            message=f"{path} is not a git repo",
            severity="critical",
        )
    return CheckResult(name=f"{name}_reachable", passed=True, message="OK")


def check_repo_freshness(
    path: Path, name: str, max_hours: float = MAX_REPO_AGE_HOURS
) -> CheckResult:
    ...
    try:
        ...
        if result.returncode != 0:
            return CheckResult(
                name=f"{name}_fresh", passed=False, message="git log failed", severity="warning"
            )
        ...
        return CheckResult(
            name=f"{name}_fresh", passed=True, message=f"{age_hours:.1f}h since last commit"
        )
...
def check_signal_bus() -> CheckResult:
    ...
    if not SIGNAL_BUS.exists():
        return CheckResult(
            name="signal_bus", passed=False, message="Signal bus file missing", severity="warning"
        )
    if not os.access(SIGNAL_BUS, os.W_OK):
        return CheckResult(
            name="signal_bus", passed=False, message="Signal bus not writable", severity="critical"
        )
    ...
    if age_hours > MAX_SIGNAL_BUS_AGE_HOURS:
        return CheckResult(
            name="signal_bus",
            passed=False,
            message=f"Signal bus stale ({age_hours:.1f}h)",
            severity="warning",
        )
...
def check_api_key(env_var: str) -> CheckResult:
    ...
    if len(val) < 10:
        return CheckResult(
            name=f"api_key_{env_var}",
            passed=False,
            message=f"${env_var} suspiciously short",
            severity="warning",
        )
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as response:
            if response.status == 200:
                return CheckResult(
                    name=f"provider_health_{provider}", passed=True, message="API reachable"
                )
            return CheckResult(
                name=f"provider_health_{provider}",
                passed=False,
