...
def emit(
    ...
    # -- telemetry -------------------------------------------------------
        ...
        if not channel or not telemetry_title or not source_skill:
            return EffectorResult(
                success=False, message="telemetry requires: channel, title, source_skill"
            )
        ...
        return EffectorResult(
            success=True, message=f"Telemetry logged: {channel} - {telemetry_title}"
        )
    ...
    # -- interphase_close ------------------------------------------------
        ...
        if not shipped or not tomorrow or not open_threads or not nudges or not day_score:
            return EffectorResult(
                success=False,
                message="interphase_close requires: shipped, tomorrow, open_threads, nudges, day_score",
            )
        if note_date:
            try:
