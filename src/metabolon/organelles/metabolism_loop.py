...
def verify_fix(state: MetabolismState) -> dict:
    ...
    healed_fingerprints = {r["fingerprint"] for r in repairs if r.get("accepted", False)}

    # Remaining unresolved infections
    remaining = [inf for inf in infections if inf["fingerprint"] not in healed_fingerprints]
...
def sweep_for_issues(state: MetabolismState) -> dict:
    ...
    if stimuli:
        ...
        for tool_name in candidates[:5]:  # cap at 5 to stay bounded
            ...
            valence_str = (
                f"{emotion.valence:.3f}"
                if emotion and emotion.valence is not None
...
def report(state: MetabolismState) -> dict:
    ...
    if repairs:
        ...
        for r in rejected:
            lines.append(f"- UNRESOLVED: **{r['tool']}** — {r.get('gate_reason', 'unknown')}")
        lines.append("")
    ...
    if not infections and not sweep:
        lines += [
            "## Verdict",
            "System healthy. No chronic infections or low-fitness tools detected.",
            "",
        ]
