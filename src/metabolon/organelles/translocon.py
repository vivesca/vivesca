...
def _direct_api(prompt: str, model: str = "glm-4.7") -> dict:
    ...
    req = urllib.request.Request("https://open.bigmodel.cn/api/anthropic/v1/messages", body)
...
def _build_goose_cmd(model: str, prompt: str, recipe: str | None = None) -> list[str]:
    ...
    cmd = ["goose", "run", "-q", "--no-session", "--provider", "glm-coding", "--model", model]
    if recipe:
        cmd += ["--recipe", recipe]
    cmd += ["-t", prompt]
    return cmd
...
def dispatch_stats(count: int = 50) -> dict:
    ...
    for provider in provider_counts:
        provider_success[provider] = sum(
            1 for e in entries if e.get("provider") == provider and e.get("exit") == 0
        )
