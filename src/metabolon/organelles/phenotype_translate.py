...
class SyncResult:
    ...
    @property
    def ok(self) -> bool:
        return not self.symlinks_failed and not self.integrin_issues and self.gemini_md_ok
    ...
    def summary(self) -> str:
        ...
        # Skills
        lines.append(
            f"Skills{mode}: {self.skills_synced} synced to ~/.agents/skills/ + ~/.claude/skills/."
        )
        ...
        if self.integrin_issues:
            issues_desc = "; ".join(f"{i['path']} ({i['problem']})" for i in self.integrin_issues)
...
def sync_phenotype(
    *,
    ...
    gemini_md_ok = (
        gemini_md_path.is_symlink() and gemini_md_path.resolve() == phenotype_md.resolve()
    )
