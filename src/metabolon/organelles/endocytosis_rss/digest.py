...
def _parse_theme_json(raw: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise ValueError(f"LLM returned non-JSON (first 200 chars): {raw[:200]!r}") from exc
...
def _recall_weekly_digests(month: str) -> list[tuple[str, str]]:
    ...
    for path in sorted(chemosensory.glob(f"weekly-ai-digest-{year}-W*.md")):
        ...
        if week_date.strftime("%Y-%m") == month or (week_date.month == int(month_num)):
...
def metabolize_digest(
    ...
    briefs = [
        synthesize_theme(model_id, theme, articles, log_entries) for theme in identified_themes
    ]
