...
3. **Data** — `~/epigenome/chromatin/GARP RAI Quiz Tracker.md` (human-readable stats + history) and `~/epigenome/chromatin/.garp-fsrs-state.json` (FSRS scheduling state). Source material in `~/epigenome/chromatin/GARP RAI Module {1-5} - Raw Content.md`.
...
**If CLI fails**, fall back: read `~/epigenome/chromatin/GARP RAI Quiz Tracker.md` + `~/epigenome/chromatin/.garp-fsrs-state.json` manually.
...
**IMPORTANT: All source files must be read using absolute paths (`/Users/terry/epigenome/chromatin/...`), never `~/epigenome/chromatin/...`. Bash grep fails silently on paths with spaces — always use the Read or Grep tool with the full path.**

1. `/Users/terry/epigenome/chromatin/GARP RAI Definition Drills.md` — **mandatory when session plan shows `[drill]` tag.** Contains comparison tables, "exam trap" callouts, and "key distinction" sections. Read the relevant section and specifically test the documented trap pattern. **If a drill entry exists and you skip it, the question quality is compromised.**
2. `/Users/terry/epigenome/chromatin/GARP RAI Review Questions.md` — official GARP questions closest to exam voice. Search for the relevant module/chapter section and use or adapt these questions before inventing new ones.
3. `/Users/terry/epigenome/chromatin/GARP RAI Fairness Scenarios Drill.md` — **mandatory for M3-demographic-parity, M3-predictive-rate-parity, M3-equal-opportunity, M3-equalized-odds, M3-individual-fairness** (the 5 fairness measure topics). 10 curated scenarios with keyed answers. Use these scenarios (or variations) instead of inventing from scratch.
4. `/Users/terry/epigenome/chromatin/GARP RAI Practice Exam.md` — exam-style questions (save for mock/ramp phase)
...
**Before generating:** Read `~/epigenome/chromatin/GARP RAI Trap Patterns.md` and grep for the topic. If a pattern entry exists, the question MUST target that exact failure mode — not a generic question on the topic. This step is mandatory, not optional.
...
- **Scenario-based where possible.** Model stems after the Practice Exam (`/Users/terry/epigenome/chromatin/GARP RAI Practice Exam.md`): "A bank uses...", "An analyst is...", "A company is...".
...
- Check `~/epigenome/chromatin/GARP RAI Trap Patterns.md` — name the pattern if it matches
...
- If any new mistake patterns emerged, append to `~/epigenome/chromatin/GARP RAI Trap Patterns.md`
...
- **Quiz tracker:** `~/epigenome/chromatin/GARP RAI Quiz Tracker.md`
- **FSRS state:** `~/epigenome/chromatin/.garp-fsrs-state.json`
- **Review questions:** `~/epigenome/chromatin/GARP RAI Review Questions.md` — official GARP questions, first choice for sourcing
- **Fairness scenarios:** `~/epigenome/chromatin/GARP RAI Fairness Scenarios Drill.md` — 10 curated scenarios with keyed answers
- **Definition drills:** `~/epigenome/chromatin/GARP RAI Definition Drills.md` — comparison tables + exam traps for weak topics
- **Practice exam:** `~/epigenome/chromatin/GARP RAI Practice Exam.md`
...
- **GARP portal:** `https://garplearning.benchprep.com/app/rai26` — login flow + navigation gotchas: `~/metabolon/loci/solutions/garp-portal-navigation.md`
