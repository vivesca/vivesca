...
| Tool gotcha | `~/germline/loci/solutions/` |
...
**MEMORY.md ≥145 lines →** downregulate by recurrence signal, not by position. Check `hits:` and `last-seen:` in memory file frontmatter (updated automatically by dendrite.py on every Read). Lowest hits + oldest last-seen = downregulate candidate. Move to `~/germline/loci/solutions/memory-overflow.md` (reversible — re-promote if topic resurfaces).
