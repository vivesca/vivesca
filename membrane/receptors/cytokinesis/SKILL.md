...
9. **Mark/note content references client-named hypothetical or pre-act deliberation?** Did any mark, finding, profile note, or session log filed this slot reference a client-named hypothetical, deliberation, or pre-act consideration — even when no actual client content was discussed and the act was never taken? If yes — abstract to category before commit. Replace `<client>-<system>-<contemplated-act>-<destination>` patterns with abstract category descriptors (e.g. "high-stakes scope-violation decision", "contractor data-handling deliberation", "fiduciary-scope edge-case"). The durable lesson is the pattern; the specific client-tooling-act-destination is incidental and creates a discoverable trail almost as costly as the act itself if the file is subpoenaed in any future legal/regulatory matter. Codifies failure caught in retrospective 2026-05-06-1010 §2b: user mark `user_judgment_patterns.md` Pushback-shapes section originally named "<client>-<tooling>-<act>-to-<vendor> thread"; Terry's meta-question ("should we avoid documenting anything sensitive?") surfaced the gap; sanitised to "high-stakes scope-violation decision" before commit. Detection rule: if the mark text contains a named client/vendor/system/tool string AND the underlying content is deliberation about an act (not an act that was taken with sanctioned authorisation), abstract before write. The principle generalises beyond client data — also applies to deliberation about peers, family, partners, or any third party where the deliberation itself would be sensitive in subpoena/discovery context. See `feedback_sanitise_client_named_hypotheticals.md`.
