...
```bash
quorate council "Should I accept this offer?" \
...
```bash
# Standard invocation
quorate council "question" --vault

# Quick parallel query
quorate quick "question" --vault

# With prompt file (avoids shell quoting issues for long prompts)
quorate council --prompt-file /tmp/prompt.txt --vault
```

**`--vault` is mandatory for:** any `council`, `oxford`, or architecture/review run. Auto-saves to `~/epigenome/chromatin/Councils/` with Obsidian Sync backup. Never use `--output /tmp/...` — `/tmp` doesn't survive reboot.

**For quick batch/agent-test runs:** use `-o ~/docs/solutions/agent-tests/<name>.md` — skips Obsidian sync but survives session end. Pattern: `quorate quick --domain banking -o ~/docs/solutions/agent-tests/proposal-architect.md "..."`
...
**Retrieving output:** When running from CC background with pipes (`2>&1 | tail`), quorate detects the pipe and uses SilentOutput — the pipe gets nothing until the session file path prints at the end. Always redirect to `~/tmp/` and read with `cat`:
```bash
quorate quick "..." > ~/tmp/quorate-<name>.txt 2>&1
# then after task completes:
cat ~/tmp/quorate-<name>.txt
```
Never use TaskOutput alone to retrieve quorate results — it will timeout and the output file in `/private/tmp` will vanish. `~/tmp/` survives the session.

**Don't retry if seemingly stuck.** One background job per query. If TaskOutput times out, just `cat ~/tmp/quorate-<name>.txt` — the job is likely still running or already done.
...
--no-save               # Don't auto-save to ~/.quorate/sessions/
```
...
```bash
quorate --help                    # Show all subcommands
...
- **GPT-5.4-Pro removed from council (LRN-20260308-001).** Real latency: 907s per call. Responses API models are incompatible with council timeouts. Current M1 = `gpt-5.2-pro` (3.6s). Kimi also removed — connection failures, replaced by DeepSeek-V3.2. Full notes: `~/docs/solutions/quorate-api-latency-benchmark.md`.
- **Before adding any new model to the council:** run `quorate quick "name a color" > ~/tmp/quorate-speedtest.txt` and check per-model timings. Any model >60s does not belong in rotation. Also run `pondus check "<model>" --format table` to verify pricing — `-pro` reasoning variants can cost 10-12x more than base.
...
- **403 on a new model = access restricted (allowlist-gated).** Test before upgrading: `quorate quick "test" 2>&1 | grep -i "403\|error"`. Swap to an available model or remove from rotation.
