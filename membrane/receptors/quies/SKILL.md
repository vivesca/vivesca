...
Read `~/epigenome/chromatin/G1.md` — look at the Progress section for `[next, blocking]` and `[parked]` items that might race tonight.

- If anything is genuinely urgent for tomorrow → note it explicitly below
- If it can wait → name it out loud and let it go

**Fail clause:** If G1.md is stale (>24h, check the SESSION CLOSE date) or missing, skip and go to Step 3.
...
Review the brain dump + G1.md `[next, blocking]` items. For anything that:
