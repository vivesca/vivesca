...
5. **Visual reproduction (UI bugs):** If the bug involves UI or user flows, use agent-browser to reproduce visually:
   ```bash
   agent-browser open "<url>" && agent-browser snapshot  # see current state
   agent-browser click "@ref" && agent-browser screenshot /tmp/bug-step1.png  # capture each step
   agent-browser eval "document.querySelectorAll('.error')"  # check for console errors
   ```
   Capture screenshots at each step. The screenshots are evidence — attach to findings. Absorbed from CE `reproduce-bug`.
