...
## Trigger-system freshness check

Skill trigger matching depends on `~/.claude/skill-triggers.json`, regenerated by `skill-trigger-gen.py` after every skill edit. The file rotted silently for 25 days in April 2026 due to a stale path — `mod_priming` matched against a frozen snapshot. Add this check to integrin runs:

```bash
test "$(find ~/.claude/skill-triggers.json -mtime +7)" && echo "STALE: skill-triggers.json > 7 days old — run skill-trigger-gen.py"

expected=$(grep -l "user_invocable: true" ~/germline/membrane/receptors/*/SKILL.md | wc -l)
registered=$(python3 -c "import json; print(len(json.load(open('$HOME/.claude/skill-triggers.json'))))")
[ "$expected" -gt "$registered" ] && echo "GAP: $expected user_invocable skills, $registered registered triggers"

[ -s ~/.claude/skill-trigger-gen.log ] && tail -20 ~/.claude/skill-trigger-gen.log
```

Background: `~/epigenome/marks/finding_skill_trigger_system_silent_failure.md`. Regression test at `~/germline/assays/test_skill_trigger_freshness.py`.
