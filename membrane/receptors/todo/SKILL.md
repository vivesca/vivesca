...
```bash
cyclin add --date YYYY-MM-DD "<task title>"
```
...
**When to apply a cyclin alarm:** Hard deadlines requiring action on a specific day — not `someday`, not `when:` gates, not recurring habits. Pick a time that fits the day (not just 9am default). This is a manual step — don't automate it, so the time is chosen deliberately.
