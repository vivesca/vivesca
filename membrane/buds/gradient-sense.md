...
1. Call the gradient sensor:
   ```bash
   cd ~/code/vivesca && uv run python -c "from metabolon.tools.gradient import proprioception_gradient; import json; print(json.dumps(proprioception_gradient().model_dump(), indent=2))"
   ```
