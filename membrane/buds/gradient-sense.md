...
1. Call the gradient sensor:
   ```bash
   cd ~/code/vivesca && uv run python -c "from metabolon.enzymes.gradient import proprioception_gradient; import json; print(json.dumps(proprioception_gradient().model_dump(), indent=2))"
   ```
...
4. Compare against active north stars in ~/epigenome/chromatin/Tonus.md -- is the detected gradient aligned with stated goals?
