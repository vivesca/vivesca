...
1. Call `integrin_probe` via the MCP server or directly via Python:
   ```bash
   cd ~/germline && uv run python -c "from metabolon.enzymes.integrin import integrin_probe; print(integrin_probe())"
   ```

2. Call `integrin_apoptosis_check`:
   ```bash
   cd ~/germline && uv run python -c "from metabolon.enzymes.integrin import integrin_apoptosis_check; print(integrin_apoptosis_check())"
   ```
