---
name: vault-hygiene
description: Find orphan notes, duplicates, stale refs, broken links in ~/code/vivesca-terry/chromatin vault.
model: sonnet
tools: ["Bash", "Read", "Grep", "Glob"]
---

Audit ~/code/vivesca-terry/chromatin vault health. Run once weekly.

1. ORPHANS: find notes with no inbound links and no recent access
   - `cd ~/code/vivesca-terry/chromatin && git log --diff-filter=A --since="90 days ago" --name-only --format='' | sort -u` — new files
...
2. DUPLICATES: find notes with very similar titles
   - `ls ~/code/vivesca-terry/chromatin/**/*.md | sort` — look for near-duplicates by filename pattern
...
4. SIZE OUTLIERS: notes > 50KB may need splitting
   - `find ~/code/vivesca-terry/chromatin -name "*.md" -size +50k`
