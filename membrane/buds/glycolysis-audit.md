...
1. Scan for LLM call patterns across the codebase:
   ```bash
   cd ~/code/vivesca && grep -rn "synthesize\|_acquire_catalyst\|llm\.query\|max20\|anthropic\|_llm_query\|llm_call" cytoplasm/ membrane/cytoskeleton/ --include="*.py" | grep -v __pycache__
   ```
