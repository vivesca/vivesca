...
def build_command(provider: ProviderConfig, prompt: str, timeout: int = 7200) -> ProviderCommand:
    ...
    if provider.harness == "claude":
        env.update({"ANTHROPIC_API_KEY": api_key, "ANTHROPIC_BASE_URL": provider.url, "ANTHROPIC_DEFAULT_SONNET_MODEL": provider.model, "CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC": "1", "API_TIMEOUT_MS": "3000000"})
        args = ["claude", "--print", "--no-session-persistence", "--dangerously-skip-permissions", "-p", prompt]
    elif provider.harness == "codex":
        env["OPENAI_API_KEY"] = api_key
        args = ["codex", "exec", "--dangerously-bypass-approvals-and-sandbox", "--skip-git-repo-check", prompt]
        ...
        args = ["goose", "run", "-q", "--no-session", "--provider", "anthropic", "--model", provider.model, "-t", prompt]
