...
1. **Load session state.** Read ~/tmp/pulse-manifest.md (memory of prior waves), ~/notes/North Star.md (includes meta goal), ~/notes/Praxis.md (head 80), ~/notes/Tonus.md. Run `date`.
...
## Taste rules
- **No duplicates.** Check ~/notes/ for existing files. Update, don't create "v2."
...
def post_acta_summary(total_waves: int, stop_reason: str):
    ...
    try:
        ...
        acta.post(
            ...
            f"Check ~/notes/Pulse Reports/ for details.",
            sender="pulse",
            to="terry",
            severity="info",
