...
Plus individual effectors:
  ...
  emit_reminder    -- set Due alarm (wraps moneo CLI)
...
NOTES = os.path.expanduser("~/code/vivesca-terry/chromatin")
...
@tool(
    name="emit_reminder",
    description="Set a Due app reminder via moneo.",
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False),
)
...
def emit_vault_note(
    ...
    """Write to a vault note. Path relative to ~/code/vivesca-terry/chromatin/.
