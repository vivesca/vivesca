...
This tool implements the sensing half: reads signals across three sensor arrays
(lustro relevance log, tool invocation log, search query log), detects which
topic domains are co-trending across multiple sensors, and reports the polarity
vector — the domain the organism is drifting toward.

What it does NOT do (yet): amplification/locking. It surfaces the gradient.
The organism then decides whether to reinforce it into a stable orientation.
...
_TOOL_DOMAINS: dict[str, str] = {
    ...
    "checkpoint_list": "career_consulting",
    "checkpoint_set": "career_consulting",
...
def _topology_weight(active_sensors: set[str]) -> tuple[float, str]:
...
def _sense_signals(days: int) -> dict[str, int]:
