...
### Consulting demos (build, not prose)
...
- [ ] `golem [t-ca07] --provider volcano --max-turns 40 "Build a provider cost tracker at ~/germline/effectors/golem-cost. Python CLI. Reads ~/.local/share/vivesca/golem.jsonl, extracts provider + duration + exit_code per task. Computes: cost per provider (duration × estimated $/min), success rate, avg duration, tasks/hour. Output as table + optional --json. Add --since flag for date filtering. Commit."`
- [ ] `golem [t-ca08] --provider codex --max-turns 40 "Build a regulatory document processing Temporal demo at ~/germline/effectors/temporal-golem/demos/regulatory-filing.py. Workflow: 1) ingest_circular activity (mock fetch from HKMA/SFC) 2) classify_document activity (mock — categorize as guidance/circular/consultation) 3) extract_obligations activity (mock NLP extraction of required actions) 4) impact_assessment activity (mock — flag affected business lines) 5) generate_action_items activity (mock — produce compliance checklist). Each with retry/timeout. Dispatch script + README. Commit."`
- [ ] `golem [t-ca09] --provider gemini --max-turns 40 "Build a trade settlement Temporal demo at ~/germline/effectors/temporal-golem/demos/trade-settlement.py. Workflow: 1) validate_trade activity (check fields) 2) enrichment activity (mock market data lookup) 3) netting activity (mock bilateral netting) 4) settlement_instruction activity (mock SWIFT MT54x generation) 5) confirmation activity. Each activity has realistic timeouts and retry policies. Include a dispatch script that submits sample trades. README.md. Commit."`
