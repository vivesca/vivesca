...
### Consulting arsenal (compounds for Capco + job search)

- [ ] `golem [t-ca01] --provider zhipu --max-turns 50 "Write a detailed consulting case study at ~/notes/Writing/Blog/Published/ai-agent-orchestration-patterns.md. Topic: 'AI Agent Orchestration Patterns for Banking Operations'. Draw from real experience — golem-daemon, Temporal, Hatchet. Cover: why banks need durable task execution, per-provider rate limiting as a pattern for multi-vendor AI, crash recovery for compliance workflows, cost optimization via provider rotation. Include architecture diagrams as ASCII. Write as a practitioner, not a vendor. 2000-3000 words. Commit."`
- [ ] `golem [t-ca02] --provider volcano --max-turns 50 "Write a consulting framework doc at ~/notes/Writing/Consulting/ai-governance-operating-model.md. Title: 'AI Governance Operating Model for Regulated Financial Institutions'. Sections: 1) Three-lines-of-defense adapted for AI 2) Model risk management for LLM applications 3) Vendor concentration risk when using multiple AI providers 4) Incident response playbook for AI failures 5) Board reporting template. Target audience: CRO/CTO at mid-tier APAC banks. 3000 words. Practical, not academic. Commit."`
- [ ] `golem [t-ca03] --provider gemini --max-turns 40 "Write a technical brief at ~/notes/Writing/Consulting/temporal-for-banking-workflows.md. Title: 'Temporal for Banking Workflow Orchestration: A Technical Assessment'. Cover: what Temporal is, comparison to Step Functions/Camunda/Airflow, banking use cases (trade settlement, KYC/AML pipelines, regulatory reporting), deployment considerations (on-prem vs cloud, data residency), proof-of-concept approach. Include a concrete PoC architecture for a KYC document processing pipeline. 2000 words. Commit."`
- [ ] `golem [t-ca04] --provider infini --max-turns 40 "Write a consulting one-pager at ~/notes/Writing/Consulting/ai-cost-optimization-banking.md. Title: 'AI Cost Optimization for Banks: Multi-Provider Strategy'. Cover: why single-provider lock-in is risky, how to build provider rotation (draw from golem experience with zhipu/infini/volcano/gemini/codex), rate-limit management patterns, cost per token benchmarks, fallback chains for resilience. Include a decision matrix for provider selection. 1500 words. Commit."`
- [ ] `golem [t-ca05] --provider codex --max-turns 40 "Write a consulting POV at ~/notes/Writing/Consulting/agentic-ai-risk-framework.md. Title: 'Risk Framework for Agentic AI in Financial Services'. Cover: what makes agentic AI different from chatbots (autonomy, tool use, multi-step), new risk categories (goal drift, tool misuse, cascading failures, data leakage via tools), control framework (human-in-the-loop patterns, circuit breakers, audit trails), regulatory expectations (HKMA, MAS, BIS). Draw from hands-on agent building experience. 2500 words. Commit."`
- [ ] `golem [t-ca06] --provider zhipu --max-turns 40 "Build a Temporal demo workflow at ~/germline/effectors/temporal-golem/demos/kyc-pipeline.py. Simulates a KYC document processing pipeline: 1) receive_documents activity (mock) 2) extract_entities activity (mock NER) 3) sanctions_screening activity (mock OFAC check) 4) risk_scoring activity (mock) 5) human_review_gate (waits for signal). Wire it as a Temporal workflow with proper retry policies, timeouts, and heartbeats. Include a README.md explaining the demo. This is for consulting demos — must look professional. Commit."`
