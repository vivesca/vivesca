...
git clone https://github.com/terry-li-hm/notes.git ~/notes
```
...
The vault at `~/notes` is a git repo with `.obsidian/` config checked in, so plugins, settings, and themes transfer automatically.

1. Open Obsidian → Add vault → `~/notes`
2. Plugins will show as installed (they live in `~/notes/.obsidian/plugins/`)
...
# Vault
ls ~/notes | wc -l                     # should have hundreds of notes
ls ~/notes/.obsidian/plugins | wc -l  # should be ~17 plugins
