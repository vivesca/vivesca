...
- Runs `ob sync --continuous --path /Users/terry/epigenome/chromatin`
...
Set `"sync": false` in `~/epigenome/chromatin/.obsidian/core-plugins.json` to avoid duplicate syncing. The headless daemon handles it. To re-enable desktop sync (e.g., travelling): flip back to `true` and restart Obsidian.
