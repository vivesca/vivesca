...
The wrap skill wrote session logs to `~/notes/Daily/YYYY-MM-DD.md` but the vault stores daily notes flat at `~/notes/YYYY-MM-DD.md`. Morning and daily skills read the flat path — so all wrap entries were invisible to them.
...
Standardised all three skills (wrap, daily, morning) to use `~/notes/Daily/YYYY-MM-DD.md`. Moved 33 flat daily notes into `~/notes/Daily/`. Merged content where both versions existed.
