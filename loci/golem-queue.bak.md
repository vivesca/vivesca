...
- [ ] `golem [t-52bf6c] --provider volcano --max-turns 30 "Health check: wacli-ro, soma-activate, update-coding-tools.sh, centrosome, circadian-probe.py, chromatin-backup.py, quorum, commensal, plan-exec, nightly. For each: run --help, ast.parse if Python, check shebang. Fix broken ones. Commit."`
...
- [ ] `golem [t-00266c] --provider volcano --max-turns 30 "Health check: health-check, regulatory-capture, disk-audit, wacli-ro, search-guard, cookie-sync, assay, efferens, hetzner-bootstrap.sh, electroreception. For each: run --help, ast.parse if Python, check shebang. Fix broken ones. Commit."`
...
- [ ] `golem [t-16dfd8] --provider volcano --max-turns 30 "Health check: synthase, golem-daemon-wrapper.sh, coverage-map, cg, soma-pull, search-guard, orphan-scan, test-fixer, complement, client-brief. For each: run --help, ast.parse if Python, check shebang. Fix broken ones. Commit."`
