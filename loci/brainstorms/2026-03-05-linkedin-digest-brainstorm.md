...
A daily automated LinkedIn feed digest. **`theoros`** — a Rust CLI — scrapes the top 10 posts from the LinkedIn feed via `agent-browser`, calls the Claude API to filter and summarise for relevance, and writes a structured vault note to `~/code/epigenome/chromatin/LinkedIn/`. A LaunchAgent runs this at 8am daily. A skill wraps the script for manual invocation.
...
| Vault path | `~/code/epigenome/chromatin/LinkedIn/YYYY-MM-DD LinkedIn Digest.md` | Grouped, searchable |
...
- **Vault location:** `~/code/epigenome/chromatin/LinkedIn/`
