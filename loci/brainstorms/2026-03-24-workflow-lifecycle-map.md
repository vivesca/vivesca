...
| ecdysis | Weekly note (~/epigenome/chromatin/Weekly/YYYY-Www.md) |
| mitosis | Monthly Review note (~/epigenome/chromatin/Monthly Review - YYYY-MM.md) + Praxis.md updated |
| meiosis | Quarterly note (~/epigenome/chromatin/Quarterly/YYYY-QX.md) + Life OS updated |
...
6. **interphase "what shipped" → expression sparks**: daily ship summary dies in the daily note; no mechanism to route it into `~/epigenome/chromatin/Consulting/_sparks.md` for weekly forging.
...
4. **expression intelligence worker → ecdysis**: the weekly intelligence brief in `~/epigenome/chromatin/Consulting/_weekly/` is never referenced in weekly planning (ecdysis) — two parallel views of "what happened this week" with no connection.
...
- `~/epigenome/chromatin/Consulting/_weekly/YYYY-WNN.md` intelligence briefs — filed and forgotten unless Terry reads them manually
...
1. **phagocytosis → expression sparks**: articles and insights saved to vault could directly seed `~/epigenome/chromatin/Consulting/_sparks.md`, but no mechanism routes them there. The connection is manual at best.
...
| differentiation | Gym log file (~/epigenome/chromatin/Health/Gym Log - YYYY-MM-DD.md) |
...
1. **Gym progression analysis**: gym logs accumulate in ~/epigenome/chromatin/Health/ but no skill reads them periodically for plateau detection or progression analysis. Differentiation reads the LATEST log, not trends.
...
| Cell | Output |
|------|--------|
| polarization | Copia report (~/epigenome/chromatin/Copia Reports/YYYY-MM-DD.md) + Praxis.md review items + self-sufficient outputs archived |
...
3. **polarization quality → expression**: copia reports track what was produced, but don't automatically seed `~/epigenome/chromatin/Consulting/_sparks.md` with produced IP. Two parallel production pipelines with no connection.
...
- `~/epigenome/chromatin/Copia Reports/YYYY-MM-DD.md` — filed and not reviewed by any other skill
...
| Cell | Output |
|------|--------|
| quorum | Council output in ~/epigenome/chromatin/Councils/ + decision recommendation |
| proofreading | Top 3 challenges ranked by severity (chat only) |
| transcription-factor | Decision note (~/epigenome/chromatin/decisions/YYYY-MM-DD-<slug>.md) |
...
2. **Council performance**: quorum outputs accumulate in ~/epigenome/chromatin/Councils/ but no skill reviews them for quality, consistency, or whether the recommended decisions turned out to be correct.
...
- `~/epigenome/chromatin/Councils/` → accumulates without longitudinal review
- `~/epigenome/chromatin/decisions/` → reviewed only via transcription-factor bouncer, not periodically
...
- `~/epigenome/chromatin/Meta/Analyze Telemetry.md` (phagocytosis) — write-only
...
`expression` runs weekly and consumes `~/epigenome/chromatin/Consulting/_sparks.md`. The sparks are supposed to be populated by a daily kinesin agent. But:
