...
| ecdysis | Weekly note (~/code/epigenome/chromatin/Weekly/YYYY-Www.md) |
| mitosis | Monthly Review note (~/code/epigenome/chromatin/Monthly Review - YYYY-MM.md) + Praxis.md updated |
| meiosis | Quarterly note (~/code/epigenome/chromatin/Quarterly/YYYY-QX.md) + Life OS updated |
...
6. **interphase "what shipped" → expression sparks**: daily ship summary dies in the daily note; no mechanism to route it into `~/code/epigenome/chromatin/Consulting/_sparks.md` for weekly forging.
...
4. **expression intelligence worker → ecdysis**: the weekly intelligence brief in `~/code/epigenome/chromatin/Consulting/_weekly/` is never referenced in weekly planning (ecdysis) — two parallel views of "what happened this week" with no connection.
...
- `~/code/epigenome/chromatin/Consulting/_weekly/YYYY-WNN.md` intelligence briefs — filed and forgotten unless Terry reads them manually
...
1. **phagocytosis → expression sparks**: articles and insights saved to vault could directly seed `~/code/epigenome/chromatin/Consulting/_sparks.md`, but no mechanism routes them there. The connection is manual at best.
...
| differentiation | Gym log file (~/code/epigenome/chromatin/Health/Gym Log - YYYY-MM-DD.md) |
...
1. **Gym progression analysis**: gym logs accumulate in ~/code/epigenome/chromatin/Health/ but no skill reads them periodically for plateau detection or progression analysis. Differentiation reads the LATEST log, not trends.
...
| Cell | Output |
|------|--------|
| polarization | Copia report (~/code/epigenome/chromatin/Copia Reports/YYYY-MM-DD.md) + Praxis.md review items + self-sufficient outputs archived |
...
3. **polarization quality → expression**: copia reports track what was produced, but don't automatically seed `~/code/epigenome/chromatin/Consulting/_sparks.md` with produced IP. Two parallel production pipelines with no connection.
...
- `~/code/epigenome/chromatin/Copia Reports/YYYY-MM-DD.md` — filed and not reviewed by any other skill
...
| Cell | Output |
|------|--------|
| quorum | Council output in ~/code/epigenome/chromatin/Councils/ + decision recommendation |
| proofreading | Top 3 challenges ranked by severity (chat only) |
| transcription-factor | Decision note (~/code/epigenome/chromatin/decisions/YYYY-MM-DD-<slug>.md) |
...
2. **Council performance**: quorum outputs accumulate in ~/code/epigenome/chromatin/Councils/ but no skill reviews them for quality, consistency, or whether the recommended decisions turned out to be correct.
...
- `~/code/epigenome/chromatin/Councils/` → accumulates without longitudinal review
- `~/code/epigenome/chromatin/decisions/` → reviewed only via transcription-factor bouncer, not periodically
...
- `~/code/epigenome/chromatin/Meta/Analyze Telemetry.md` (phagocytosis) — write-only
...
`expression` runs weekly and consumes `~/code/epigenome/chromatin/Consulting/_sparks.md`. The sparks are supposed to be populated by a daily kinesin agent. But:
