...
categorise merchants (YAML map, prefix match)
    ↓
write per-statement markdown:  ~/notes/Spending/YYYY-MM-bank.md
    ↓
write/update monthly summary:  ~/notes/Spending/YYYY-MM-summary.md
    ↓
move PDF to vault:             ~/notes/Spending/statements/YYYY-MM-bank.pdf
    ↓
...
- **Recurrence detection:** a merchant is "recurring" if it appears in 2+ consecutive months with amount variance <10%. The monitor reads prior months' parsed markdown files from `~/notes/Spending/` to determine recurrence. New recurring charges not in the expected list are flagged for review.
...
Before processing a PDF, compute SHA-256 hash of the file. Check against `~/notes/Spending/.processed` (one hash per line). If already processed, skip. After successful processing, append hash.
...
Parsed markdown contains merchant-level spending data (no card numbers, no addresses). Original PDFs contain full PII (name, address, partial card number). Both live in `~/notes/Spending/`.
