...
All consulting IP lives under `~/notes/Consulting/`:
...
**Inputs:**
- Lustro output from previous 24h (`~/notes/AI News Log.md`, last 24h entries)
...
- Thalamus landscape notes (`~/notes/Thalamus.md`) — lead agent synthesises directly, no dialexis dependency
...
**Worker output rules:**
- Each worker creates files directly in the appropriate `~/notes/Consulting/` subdirectory
...
1. **Create directory structure** — `~/notes/Consulting/` with subdirectories
