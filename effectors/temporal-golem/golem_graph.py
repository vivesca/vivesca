#!/usr/bin/env python3
"""LangGraph-based golem agent for Temporal worker.
...
SYSTEM_PROMPT = """You are a golem — a task execution agent. You receive a task and execute it precisely.
...
def execute_node(state: GolemState) -> dict:
...
def should_continue(state: GolemState) -> str:
...
def verify_node(state: GolemState) -> dict:
...
def review_node(state: GolemState) -> dict:
...
def build_golem_graph():
    """Build and compile the golem agent graph."""
    graph = StateGraph(GolemState)
...
def run_golem_graph(task: str, provider: str = "zhipu") -> dict:
