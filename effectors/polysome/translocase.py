...
# Per-provider concurrency limits (matching ribosome-daemon config)
PROVIDER_LIMITS = {
    "zhipu": 8,
    "infini": 8,
    "volcano": 16,
...
# Serialize merges: only one ribosome merges at a time (file lock).
...
# Maximum time a single ribosome invocation may run
...
def _git_pull_ff_only(repo_root: str) -> None:
    """Pull latest changes before ribosome runs to pick up CC-written test files."""
    try:
...
def _git_push(repo_root: str) -> None:
    """Push ribosome commits to origin so soma can pull without manual intervention."""
    try:
        result = _subprocess.run(
            ["git", "push"],
            capture_output=True,
...
def _merge_worktree(repo_root: str, branch_name: str, worktree_path: str) -> bool:
    ...
    Strategy:
    - Serialize via file lock so concurrent ribosomes queue instead of racing.
    - Fast-forward when main hasn't moved (common case).
    - Real 3-way merge (--no-ff) when another ribosome moved main.
...
def _detect_prior_commits(
    ...
    """Find commits from a prior killed ribosome attempt within the time window.
...
async def translate(task: str, provider: str, max_turns: int = 50) -> dict:
    ...
    # Syntax pre-check: validate ribosome script before dispatching
    try:
    ...
    # Run ribosome from repo root, not polysome subdir
    ...
    # Pull latest changes (e.g. CC-written test files) before running ribosome
    await asyncio.to_thread(_git_pull_ff_only, work_dir)

    # Snapshot before ribosome runs
    ...
    # Snapshot after ribosome runs (in worktree)
    ...
    # #5: Extract token/cost info from ribosome output
    cost_info = ""
...
# ── LangGraph translation activity ──
...
# ── Red-flag patterns that indicate ribosome may have done damage ──
...
async def chaperone(result: dict) -> dict:
    ...
    and writes a review entry to ribosome-reviews.jsonl.
    ...
    # 5. Empty stdout with success (ribosome should always produce output)
...
async def main() -> None:
    ...
    print(
        f"Temporal ribosome worker started on queue '{TASK_QUEUE}' (max_concurrent={max_concurrent})"
    )
    await worker.run()
