...
Temporal.io-based workflow orchestrator for translation task dispatch.
...
Tasks live in `~/germline/loci/translation-queue.md`:

```markdown
- [ ]  `ribosome --provider zhipu [t-abc123] "Write tests for foo.py"`
- [!!] `ribosome --provider volcano [t-def456] "High priority fix"`
```
...
1. **Execute** (`translate`) — runs ribosome subprocess, captures
   stdout/stderr/exit-code, snapshots `git diff` before and after.
2. **Review** (`chaperone`) — checks for:
...
Reviews persist to `~/germline/loci/ribosome-reviews.jsonl`.
