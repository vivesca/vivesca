#!/usr/bin/env python3
"""polysome-dispatch — read translation-queue.md and dispatch tasks via Temporal.

Reads the same queue file format as ribosome-daemon and hatchet-dispatch,
...
LOG_FILE = Path.home() / ".local" / "share" / "vivesca" / "polysome-dispatch.log"

# Runtime state: task_id -> workflow handle (tracks in-flight dispatches)
_inflight: dict[str, object] = {}

# ---------------------------------------------------------------------------
# QueueLock — file-based mutex shared with ribosome-daemon
# ---------------------------------------------------------------------------


class QueueLock:
    """Exclusive file lock on the same lock file ribosome-daemon uses."""

    _lock_path = Path.home() / ".local" / "share" / "vivesca" / "translation-queue.lock"
...
# ---------------------------------------------------------------------------
# Task-ID format (same as ribosome-daemon)
# ---------------------------------------------------------------------------

TASK_ID_RE = re.compile(r"\[t-([0-9a-f]{6})\]")

# ---------------------------------------------------------------------------
# Per-provider concurrency limits (mirrors ribosome-daemon)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Rate-limit detection (mirrors ribosome-daemon)
# ---------------------------------------------------------------------------
...
COOLDOWN_LOG = Path.home() / ".local" / "share" / "vivesca" / "ribosome-cooldowns.json"
...
def _generate_task_id() -> str:
    """Generate a task ID in t-XXXXXX format (6 hex chars, same as ribosome-daemon)."""
...
def parse_queue() -> list[tuple[int, str, str, str, int]]:
    ...
    for i, line in enumerate(lines):
        ...
        if tid_match:
            ...
            # Inject task ID after "ribosome " in the command, matching ribosome-daemon
            new_cmd = cmd.replace("ribosome ", f"ribosome [{task_id}] ", 1)
            lines[i] = line.replace(f"`{cmd}`", f"`{new_cmd}`", 1)
            cmd = new_cmd
            modified = True

        # Strip ribosome prefix, task IDs, and known flags to extract bare prompt
        prompt = re.sub(r"^ribosome\s+", "", cmd)
...
def _auto_requeue(prompt: str, provider: str) -> None:
    ...
    entry = f"- [ ] `ribosome --provider {provider} [{tid}] {prompt}`\n"
    with QueueLock():
...
def _sync_reviews() -> None:
    """Pull ribosome-reviews.jsonl from ganglion; sync germline via git."""
    import subprocess

    # Pull review log (not in git — ephemeral)
    review_src = "ganglion:~/germline/loci/ribosome-reviews.jsonl"
    review_dst = str(Path.home() / "germline" / "loci" / "ribosome-reviews.jsonl")
    with contextlib.suppress(Exception):
        subprocess.run(["rsync", "-az", review_src, review_dst], timeout=30, capture_output=True)
    # Git pull on ganglion to sync skills + polysome code
    try:
        subprocess.run(
                    ...
                    with contextlib.suppress(Exception):
                        subprocess.Popen(
                            [
                                ...
                                'send_message("[soma] polysome-dispatch: Temporal server unreachable after 5+ poll cycles")',
                            ],
...
async def show_status(json_output: bool = False) -> None:
    ...
    async for wf in client.list_workflows(
        query="WorkflowType = 'TranslationWorkflow'",
        page_size=20,
    ):
        results.append(
            {
