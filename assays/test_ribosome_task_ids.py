"""Tests for ribosome-daemon task-ID features.

Covers: ID generation, ID extraction, parse_queue ID assignment,
run_ribosome RIBOSOME_TASK_ID env var, JSONL task_id field,
...
def _load():
    """Load ribosome-daemon by exec-ing its source (not __main__)."""
    src = open(str(Path.home() / "germline/effectors/ribosome-daemon")).read()
    ns: dict = {"__name__": "ribosome_daemon_test"}
    exec(src, ns)
    return ns
...
run_ribosome = _mod["run_ribosome"]
...
class TestTaskIDRegex:
    def test_match(self):
        m = TASK_ID_RE.search("ribosome [t-a1b2c3]")
        assert m is not None
        assert m.group(1) == "a1b2c3"

    def test_no_match_no_brackets(self):
        assert TASK_ID_RE.search("ribosome --provider zhipu") is None
...
class TestParseQueueIDs:
    ...
    def _setup_queue(self, tmp_path):
        self.qf = tmp_path / "translation-queue.md"
    ...
    def test_preserves_existing_id(self):
        self.qf.write_text(
            textwrap.dedent("""\
            ## Tasks
            - [ ] `ribosome [t-abcdef] --provider zhipu "hello"`
            ## Done
        """)
        )
        result = parse_queue()
        assert len(result) == 1
        _line, _cmd, task_id = result[0]
    ...
    def test_mixed_ids(self):
        self.qf.write_text(
            textwrap.dedent("""\
            ## Tasks
            - [ ] `ribosome [t-111111] --provider zhipu "has id"`
            - [ ] `ribosome --provider volcano "no id"`
            ## Done
        """)
        )
        result = parse_queue()
        assert len(result) == 2
        _, _cmd1, tid1 = result[0]
        _, _cmd2, tid2 = result[1]
...
# ---------------------------------------------------------------------------
# 5. run_ribosome sets RIBOSOME_TASK_ID env var
# ---------------------------------------------------------------------------


class TestRunRibosomeTaskID:
    def test_env_var_set(self):
        ...
        try:
            _cmd, _exit, _tail, _dur = run_ribosome('ribosome [t-beef42] --provider zhipu "test"')
        finally:
            _mod["subprocess"].run = _real_run
        assert captured_env.get("RIBOSOME_TASK_ID") == "t-beef42"
    ...
    def test_env_var_empty_when_no_id(self):
        ...
        _mod["subprocess"].run = fake_run
        try:
            _cmd, _exit, _tail, _dur = run_ribosome('ribosome --provider zhipu "test"')
        finally:
            _mod["subprocess"].run = _real_run
        assert captured_env.get("RIBOSOME_TASK_ID") == ""
...
class TestJsonlTaskID:
    def test_task_id_in_record(self, tmp_path):
        ...
        try:
            _write_jsonl_record(
                "t-cafe00", "zhipu", 0, 42, 'ribosome [t-cafe00] --provider zhipu "x"'
            )
    ...
    def test_empty_task_id(self, tmp_path):
        ...
        _mod["JSONLFILE"] = jsonl
        try:
            _write_jsonl_record("", "zhipu", 1, 10, 'ribosome --provider zhipu "x"')
            record = json.loads(jsonl.read_text().strip())
        finally:
            _mod["JSONLFILE"] = _REAL_JSONLFILE
...
class TestMarkDoneTaskID:
    ...
    def test_done_annotation_includes_task_id(self):
        self.qf.write_text(
            textwrap.dedent("""\
            ## Tasks
            - [ ] `ribosome [t-aabbcc] --provider zhipu "test"`
            ## Done
        """)
        )
    ...
    def test_high_priority_done(self):
        self.qf.write_text(
            textwrap.dedent("""\
            ## Tasks
            - [!!] `ribosome [t-ff00ff] --provider zhipu "urgent"`
            ## Done
        """)
        )
...
class TestMarkFailedTaskID:
    ...
    def test_retry_includes_task_id_in_log(self):
        ...
        self.qf.write_text(
            textwrap.dedent("""\
            ## Tasks
            - [ ] `ribosome [t-dead00] --provider zhipu "test"`
            ## Done
        """)
        )
        _real_log = _mod["log"]
    ...
    def test_permanent_failure_with_task_id(self):
        self.qf.write_text(
            textwrap.dedent("""\
            ## Tasks
            - [ ] `ribosome [t-badbad] --provider zhipu "test"`
            ## Done
        """)
        )
        _real_log = _mod["log"]
    ...
    def test_rate_limited_retry_with_task_id(self):
        self.qf.write_text(
            textwrap.dedent("""\
            ## Tasks
            - [ ] `ribosome [t-ratel1] --provider zhipu "test"`
            ## Done
        """)
        )
        _real_log = _mod["log"]
        ...
        try:
            result = mark_failed(
                1, "Error: 429 rate limit exceeded", exit_code=2, task_id="t-ratel1"
            )
        finally:
            _mod["log"] = _real_log
        assert result["retried"] is True
...
class TestCmdStatusRunningIDs:
    def test_shows_running_task_ids(self, tmp_path, capsys):
        _mod["QUEUE_FILE"] = tmp_path / "translation-queue.md"
        _mod["RUNNING_FILE"] = tmp_path / "running.json"
        _mod["read_pid"] = lambda: 1234
        (tmp_path / "translation-queue.md").write_text("## Done\n")
        try:
            running_data = [
                {
                    "task_id": "t-abc123",
                    "provider": "zhipu",
                    "cmd": "ribosome [t-abc123] --provider zhipu do_x",
                },
                {
                    "task_id": "t-def456",
                    "provider": "volcano",
                    "cmd": "ribosome [t-def456] --provider volcano do_y",
                },
            ]
