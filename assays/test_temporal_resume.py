...
class TestDetectPriorCommits:
    ...
    def test_prior_commits_detected(self, tmp_git_repo: Path) -> None:
        """A recent ribosome-authored commit within the time window is returned."""
        # Make a commit authored by 'ribosome'
        f = tmp_git_repo / "partial.txt"
        f.write_text("partial fix\n")
        subprocess.run(
            ["git", "add", "partial.txt"], cwd=str(tmp_git_repo), check=True, capture_output=True
        )
        subprocess.run(
            ["git", "commit", "-m", "ribosome: partial fix"],
            cwd=str(tmp_git_repo),
            check=True,
            capture_output=True,
        )
        # Set the author to ribosome on the last commit
        subprocess.run(
            ["git", "commit", "--amend", "--author=ribosome <ribosome@local>", "--no-edit"],
            cwd=str(tmp_git_repo),
            check=True,
            capture_output=True,
        )
    ...
    def test_old_commits_ignored(self, tmp_git_repo: Path) -> None:
        ...
        # Make a commit authored by 'ribosome' but dated 2 hours ago
        f = tmp_git_repo / "old.txt"
        f.write_text("old work\n")
        subprocess.run(
            ["git", "add", "old.txt"], cwd=str(tmp_git_repo), check=True, capture_output=True
        )
        ...
        subprocess.run(
            ["git", "commit", "-m", "ribosome: old work", "--author=ribosome <ribosome@local>"],
            cwd=str(tmp_git_repo),
            check=True,
            capture_output=True,
            env=env,
...
class TestPromptPrepend:
    ...
    def test_prompt_prepend(self) -> None:
        ...
        prior_commits = ["abc1234 ribosome: partial fix", "def5678 ribosome: more work"]
