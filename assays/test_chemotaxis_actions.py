...
def test_extract_requires_url():
    ...
    result = chemotaxis(action="extract")
    assert result.success is False
...
def test_extract_success():
    ...
    with patch("metabolon.enzymes.chemotaxis._run_ab") as mock_ab, patch("time.sleep"):
        mock_ab.side_effect = [
            (True, "navigated"),  # open
        ...
        result = chemotaxis(action="extract", url="https://x.com", wait_ms=0)
...
def test_extract_navigation_failure():
    ...
    with patch("metabolon.enzymes.chemotaxis._run_ab") as mock_ab:
        mock_ab.return_value = (False, "Connection refused")
        result = chemotaxis(action="extract", url="https://x.com")
        assert result.success is False
...
def test_screenshot_requires_url():
    ...
    result = chemotaxis(action="screenshot")
    assert result.success is False
    assert "url" in result.error.lower()
...
def test_check_auth_requires_domain():
    ...
    result = chemotaxis(action="check_auth")
    assert result.success is False
...
def test_check_auth_authenticated():
    ...
    with patch("metabolon.enzymes.chemotaxis._run_ab") as mock_ab, patch("time.sleep"):
        ...
        result = chemotaxis(action="check_auth", domain="example.com")
        assert result.success is True
...
def test_extract_with_wait_ms():
    ...
    with patch("metabolon.enzymes.chemotaxis._run_ab") as mock_ab, patch(
        ...
        result = chemotaxis(action="extract", url="https://x.com", wait_ms=2000)
        assert result.success is True
...
def test_chemotaxis_actions_action_case_insensitive():
...
def test_check_auth_domain_with_http_prefix():
    ...
    with patch("metabolon.enzymes.chemotaxis._run_ab") as mock_ab, patch("time.sleep"):
        ...
        result = chemotaxis(action="check_auth", domain="https://example.com")
        assert result.success is True
        assert result.data["is_authenticated"] is True
...
def test_chemotaxis_result_model():
    """NavigatorResult stores fields correctly."""
    from metabolon.enzymes.chemotaxis import NavigatorResult
...
def test_cleanup_temp_screenshots():
    ...
    tmp = os.path.join(tempfile.gettempdir(), "chemotaxis_test_cleanup.png")
