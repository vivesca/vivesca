...
def _load():
    """Load x-feed-to-lustro by exec-ing its Python body."""
    source = open(Path.home() / "germline" / "effectors" / "x-feed-to-lustro").read()
    ns: dict = {"__name__": "x_feed_to_lustro"}
    exec(source, ns)
    return ns
...
def _tweet(text="A" * 150, handle="testuser", name="Test User",
           url="https://x.com/testuser/status/123", date="Mon Jan 01 12:00:00 +0000 2025",
           quoted_tweet=None):
    t = {
        "text": text,
...
def test_module_loads():
    ...
    assert str(CACHE_DIR).endswith(".cache/lustro-articles")


def test_cache_dir_under_home():
    """CACHE_DIR is under the user's home directory."""
    assert CACHE_DIR == Path.home() / ".cache" / "lustro-articles"
