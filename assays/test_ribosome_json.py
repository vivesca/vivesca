"""Tests for ribosome --json flag and summary --json output."""
...
def _json_formatter_code() -> str:
    """Extract the inline Python JSON formatter from the ribosome script."""
...
def test_json_formatter_produces_valid_json(tmp_path):
    ...
    outfile.write_text("Hello from ribosome")

    result = subprocess.run(
        [
            sys.executable,
            "-c",
            _json_formatter_code(),
            str(outfile),
            "0",
            "42",
            "zhipu",
    ...
    assert data["output"] == "Hello from ribosome"
...
def _make_log(tmp_path: Path, records: list[dict]) -> Path:
    ...
    log = tmp_path / "ribosome.jsonl"
    log.write_text("\n".join(json.dumps(r) for r in records) + "\n")
    return log


def test_summary_json_produces_valid_json(tmp_path):
    """ribosome summary --json produces valid JSON output."""
    log = _make_log(
        tmp_path,
        [
            {
            ...
            {"provider": "zhipu", "exit": 1, "duration": 60, "tests_passed": 5, "tests_failed": 2},
        ],
    )

    result = subprocess.run(
        ["bash", str(RIBOSOME), "summary", "--json", f"--log={log}"],
        capture_output=True,
        text=True,
    )
    ...
    assert (
        data["zhipu"]["tests_created"] == 7
    )  # (5+2) + (0+0) wait no: tests_passed=0,tests_failed=0 for first + tests_passed=5,tests_failed=2 for second = 0+7=7


def test_summary_json_multiple_providers(tmp_path):
    """ribosome summary --json separates stats by provider."""
    log = _make_log(
        tmp_path,
        [
            {
...
def test_summary_json_empty_log(tmp_path):
    """ribosome summary --json with empty log file returns empty object."""
    log = tmp_path / "ribosome.jsonl"
    log.write_text("")
...
def test_summary_json_missing_log(tmp_path):
    """ribosome summary --json with missing log file exits non-zero."""
...
def test_summary_json_skips_malformed_lines(tmp_path):
    """ribosome summary --json skips malformed JSONL lines gracefully."""
    log = tmp_path / "ribosome.jsonl"
    log.write_text(
        json.dumps(
            {"provider": "zhipu", "exit": 0, "duration": 10, "tests_passed": 0, "tests_failed": 0}
        )
        + "\n"
        "not json at all\n"
        "{broken\n"
...
def test_summary_json_with_recent_flag(tmp_path):
    """ribosome summary --json --recent N only counts last N entries."""
    records = [
        {"provider": "zhipu", "exit": 0, "duration": i * 10, "tests_passed": 0, "tests_failed": 0}
        for i in range(10)
    ]
    # Add volcano entries at the end
    records.append(
        {"provider": "volcano", "exit": 0, "duration": 100, "tests_passed": 5, "tests_failed": 0}
    )
    records.append(
        {"provider": "volcano", "exit": 1, "duration": 50, "tests_passed": 0, "tests_failed": 2}
    )
    log = _make_log(tmp_path, records)

    result = subprocess.run(
        ["bash", str(RIBOSOME), "summary", "--json", "--recent", "2", f"--log={log}"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0, result.stderr
...
def test_summary_json_single_line_output(tmp_path):
    """ribosome summary --json output is single-line (pipe-friendly)."""
    log = _make_log(
        tmp_path,
        [
            {
...
def test_help_shows_json_flag():
    """ribosome --help shows --json in usage."""
    result = subprocess.run(
        ["bash", str(RIBOSOME), "--help"],
        capture_output=True,
        text=True,
    )
...
def test_help_shows_json_in_summary():
    """ribosome summary --help shows --json flag."""
    # ribosome summary doesn't have its own --help; check the main help
    result = subprocess.run(
        ["bash", str(RIBOSOME), "--help"],
        capture_output=True,
        text=True,
...
def test_summary_text_output_not_json(tmp_path):
    """ribosome summary (without --json) produces human-readable table, not JSON."""
    log = _make_log(
        tmp_path,
        [
            {
    ...
    result = subprocess.run(
        ["bash", str(RIBOSOME), "summary", f"--log={log}"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0, result.stderr
