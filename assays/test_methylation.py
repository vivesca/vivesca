...
methylation_path = Path("/Users/terry/germline/effectors/methylation")
...
def test_write_proposal(mock_paths: dict, monkeypatch: pytest.MonkeyPatch):
    """write_proposal creates the expected file."""
    monkeypatch.setattr(methylation, "TMP_DIR", mock_paths["tmp_dir"])
...
def test_write_proposal_empty_patterns(mock_paths: dict, monkeypatch: pytest.MonkeyPatch):
...
def test_write_hybridization_proposal_new_file(mock_paths: dict, monkeypatch: pytest.MonkeyPatch):
...
def test_write_hybridization_proposal_appends(mock_paths: dict, monkeypatch: pytest.MonkeyPatch):
...
def test_main_no_patterns(mock_paths: dict, monkeypatch: pytest.MonkeyPatch, capsys):
    ...
    monkeypatch.setattr(methylation, "INFECTION_LOG", mock_paths["data_dir"] / "nonexistent.jsonl")
    monkeypatch.setattr(methylation, "INFLAMMASOME_LOG", mock_paths["logs_dir"] / "nonexistent.log")
    monkeypatch.setattr(methylation, "LOG_FILE", mock_paths["logs_dir"] / "methylation.log")
    monkeypatch.setattr(methylation, "INFLAMMASOME_PY", mock_paths["inflammasome_py"])
