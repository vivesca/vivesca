...
class TestMitosisCommands:
    """Tests for mitosis CLI commands."""

    @patch("metabolon.organelles.mitosis.sync")
    def test_mitosis_sync(self, mock_sync, tmp_path):
        """Test mitosis sync command."""
        mock_report = MagicMock()
        mock_report.ok = True
        mock_report.results = []
        mock_report.summary = "Sync complete"
        mock_sync.return_value = mock_report

        runner = CliRunner()
        result = runner.invoke(cli, ["mitosis", "sync"])

        assert result.exit_code == 0
        mock_sync.assert_called_once()

    @patch("metabolon.organelles.mitosis.status")
    def test_mitosis_status(self, mock_status, tmp_path):
        """Test mitosis status command."""
        mock_status.return_value = {
            "reachable": True,
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["mitosis", "status"])

        assert result.exit_code == 0
        assert "REACHABLE" in result.output

    @patch("metabolon.organelles.mitosis.status")
    def test_mitosis_status_unreachable(self, mock_status, tmp_path):
        """Test mitosis status when unreachable."""
    ...
    @patch("metabolon.organelles.mitosis.smoketest")
    def test_mitosis_test_success(self, mock_smoke, tmp_path):
        """Test mitosis test command success."""
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["mitosis", "test"])

        assert result.exit_code == 0
        assert "PASS" in result.output

    @patch("metabolon.organelles.mitosis.smoketest")
    def test_mitosis_test_failure(self, mock_smoke, tmp_path):
        """Test mitosis test command failure."""
        mock_smoke.return_value = {
            "success": False,
