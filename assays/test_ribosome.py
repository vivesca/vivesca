"""Tests for ribosome effectors (ribosome-daemon, ribosome-dash, ribosome-health,
ribosome-reviewer, ribosome-validate).
...
def _load_script(name: str) -> dict:
    ...
    mod_name = f"_test_ribosome_{name.replace('-', '_')}"
    # Create a real module in sys.modules (needed for @dataclass in ribosome-health)
...
class ns_proxy:
    ...
    def __getattr__(self, name):
        try:
            ...
            raise AttributeError(f"namespace has no {name!r}") from err
...
def tmp_queue(tmp_path):
    """Create a temp translation-queue.md with sample tasks."""
    q = tmp_path / "translation-queue.md"
    q.write_text(
        "# Ribosome Queue\n\n"
        "## Pending\n\n"
        '- [ ] `ribosome "Write tests for foo.py"`\n'
        '- [ ] `ribosome --provider zhipu "Test bar.py"`\n'
        '- [ ] `ribosome --provider volcano "Test baz.py"`\n'
        "\n## Done\n\n"
    )
    return q
...
def tmp_jsonl(tmp_path):
    """Create a temp ribosome.jsonl."""
    j = tmp_path / "ribosome.jsonl"
    records = [
...
class TestDaemonParseQueue:
    ...
    def test_parse_non_ribosome_command_skipped(self, tmp_path):
        ns = _load("ribosome-daemon")
        q = tmp_path / "q.md"
        q.write_text('- [ ] `echo hello`\n- [ ] `ribosome "real task"`\n')
        ns.QUEUE_FILE = q
        result = ns.parse_queue()
        assert len(result) == 1
...
class TestDaemonParseProvider:
    def test_explicit_provider(self):
        ns = _load("ribosome-daemon")
        assert ns.parse_provider('ribosome --provider zhipu "test"') == "zhipu"

    def test_no_provider_returns_default(self):
        ns = _load("ribosome-daemon")
        assert ns.parse_provider('ribosome "test"') == "default"

    def test_volcano_provider(self):
        ns = _load("ribosome-daemon")
        assert ns.parse_provider("ribosome --provider volcano task") == "volcano"

    def test_infini_provider(self):
        ns = _load("ribosome-daemon")
        assert ns.parse_provider("ribosome --provider infini task") == "infini"
...
class TestDaemonDispatchMigration:
    def test_cooldown_provider_migrates_to_any_free_provider(self):
        ns = _load("ribosome-daemon")
        cmd = 'ribosome [t-abc123] --provider infini "test baz.py"'
    ...
    def test_non_cooldown_provider_keeps_affinity(self):
        ns = _load("ribosome-daemon")
        cmd = 'ribosome [t-abc123] --provider zhipu "test bar.py"'
    ...
    def test_jsonl_record_keeps_affinity_provider_for_stats(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            jsonl_path = Path(temp_dir) / "ribosome.jsonl"
            ...
            ns._write_jsonl_record(
                ...
                0,
                42,
                'ribosome [t-abc123] --provider zhipu "test baz.py"',
...
class TestDaemonReadPid:
    ...
    def test_stale_pidfile(self, tmp_path):
        ns = _load("ribosome-daemon")
        pidfile = tmp_path / "ribosome-daemon.pid"
...
class TestDaemonMarkDone:
    def test_mark_done_replaces_checkbox(self, tmp_queue):
        ns = _load("ribosome-daemon")
        ns.QUEUE_FILE = tmp_queue
        lock_path = tmp_queue.parent / "ribosome-queue.lock"
...
class TestDaemonValidateRibosomeOutput:
    def test_passes_when_no_changes(self):
        ...
        with patch("subprocess.run", return_value=mock_result):
            passed, errors = ns.validate_ribosome_output()
        assert passed is True
        assert errors == []


class TestDaemonRunRibosome:
    """Test run_ribosome with mocked subprocess."""

    def test_run_ribosome_success(self):
        ns = _load("ribosome-daemon")
        mock_result = MagicMock()
        mock_result.returncode = 0
        ...
        mock_result.stderr = ""
        with patch("subprocess.run", return_value=mock_result):
            cmd, exit_code, _tail, _duration = ns.run_ribosome('ribosome "test"')
        assert exit_code == 0
        assert "test" in cmd

    def test_run_ribosome_failure(self):
        ns = _load("ribosome-daemon")
        mock_result = MagicMock()
        ...
        with patch("subprocess.run", return_value=mock_result):
            _, exit_code, _, _ = ns.run_ribosome('ribosome "test"')
        assert exit_code == 1


class TestDaemonRibosomeEnv:
    def test_includes_effectors_in_path(self):
        ns = _load("ribosome-daemon")
        env = ns._ribosome_env()
...
class TestDaemonRotateLogs:
    ...
    def test_rotate_large_file(self, tmp_path):
        ns = _load("ribosome-daemon")
        big_log = tmp_path / "ribosome-daemon.log"
        big_log.write_bytes(b"x" * (6 * 1024 * 1024))
        ns.LOGFILE = big_log
        ns.JSONLFILE = tmp_path / "nope.jsonl"
        ns.rotate_logs()
        assert (tmp_path / "ribosome-daemon.log.1").exists()
        assert not big_log.exists()

    def test_rotate_small_file_kept(self, tmp_path):
        ns = _load("ribosome-daemon")
        small_log = tmp_path / "ribosome-daemon.log"
...
class TestDashQueueStatus:
    def test_basic_status(self, tmp_queue):
        ...
        status_text, _last_done, _, _, _ = ns.queue_status(tmp_queue, use_color=False)
    ...
    def test_with_done_tasks(self, tmp_path):
        ...
        q.write_text(
            '- [ ] `ribosome "task A"`\n'
            '- [x] `ribosome "task B"` \u2192 exit=0\n'
            '- [!] `ribosome "task C"`\n'
        )
        # queue_status returns 5 values
        status_text, last_done, _pending, _done, _failed = ns.queue_status(q, use_color=False)
...
class TestDashMain:
    def test_help_flag(self, capsys):
        ...
        assert "ribosome-dash" in capsys.readouterr().out
...
class TestDashLastCompleted:
    ...
    def test_with_entries(self):
        ns = _load("ribosome-dash")
        out = ns.last_completed_table([('ribosome "Write tests"', "exit=0")], use_color=False)
        assert "Write tests" in out


# ===================================================================
# ribosome-health tests
# ===================================================================


class TestHealthSourceEnv:
    def test_missing_file(self, tmp_path):
        ns = _load("ribosome-health")
...
class TestHealthCheckProvider:
    def test_unknown_provider(self):
        ns = _load("ribosome-health")
        result = ns.check_provider(
            "unknown_provider", os.environ.copy(), EFFECTORS_DIR / "ribosome"
        )
    ...
    def test_missing_key(self):
        ns = _load("ribosome-health")
        result = ns.check_provider("zhipu", {}, EFFECTORS_DIR / "ribosome")
...
# ===================================================================
# ribosome-reviewer tests
# ===================================================================


class TestReviewerRun:
    def test_run_echo(self):
        ns = _load("ribosome-reviewer")
...
class TestReviewerCheckDaemonStatus:
    def test_parses_running(self):
        ns = _load("ribosome-reviewer")
        with patch.object(
            ns, "run", return_value=(0, "Daemon running (PID 1234), 5 pending tasks (zhipu:3)")
        ):
...
# ===================================================================
# ribosome-validate tests
# ===================================================================


class TestValidateFile:
    def test_valid_file(self, tmp_path):
        ns = _load("ribosome-validate")
    ...
    def test_unreadable_file(self, tmp_path):
        ...
        f.chmod(0o000)
        try:
            status, _issues = ns.validate_file(f)
            assert status == "FAIL"
        finally:
            f.chmod(0o644)
...
# ===================================================================
# ribosome (bash script) tests via subprocess
# ===================================================================


class TestRibosomeBashHelp:
    def test_help_flag(self):
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "--help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
    ...
    def test_summary_help(self):
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "summary", "--help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
class TestRibosomeBashSummary:
    def test_summary_no_log(self, tmp_path):
        log_file = tmp_path / "ribosome.jsonl"
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "summary"],
            capture_output=True,
            text=True,
            timeout=10,
            env={**os.environ, "RIBOSOME_LOG": str(log_file)},
        )
        assert r.returncode != 0
    ...
    def test_summary_recent_flag(self, tmp_path):
        ...
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "summary", "--recent", "5"],
            capture_output=True,
            text=True,
            timeout=10,
            env={**os.environ, "RIBOSOME_LOG": str(log_file)},
    ...
    def test_summary_custom_log(self, tmp_path):
        ...
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "summary", f"--log={log_file}"],
            capture_output=True,
            text=True,
            timeout=10,
            env={**os.environ, "RIBOSOME_LOG": str(tmp_path / "default.jsonl")},
        )
        assert r.returncode == 0
        assert "volcano" in r.stdout


class TestRibosomeBashJsonFlag:
    """Tests for --json flag on the main ribosome command."""
    ...
    def test_json_flag_accepted(self):
        ...
        """
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "--json", "--help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
    ...
    def test_json_with_quiet_accepted(self):
        ...
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "--json", "--quiet", "--help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
class TestRibosomeBashJsonOutput:
    """Tests for --json flag output structure on the main ribosome command.
    ...
    def test_json_output_success(self, tmp_path):
        """When ribosome succeeds, --json outputs valid JSON with expected fields."""
    ...
    def test_json_output_failure(self, tmp_path):
        """When ribosome fails, --json outputs valid JSON with non-zero exit_code."""
...
class TestRibosomeSummaryJson:
    ...
    def test_summary_json_outputs_valid_json(self, tmp_path):
        ...
        log_file.write_text("\n".join(json.dumps(r) for r in records) + "\n")
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "summary", "--json"],
            capture_output=True,
            text=True,
            timeout=10,
            env={**os.environ, "RIBOSOME_LOG": str(log_file)},
    ...
    def test_summary_json_with_recent(self, tmp_path):
        ...
        log_file.write_text("\n".join(json.dumps(r) for r in records) + "\n")
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "summary", "--json", "--recent", "1"],
            capture_output=True,
            text=True,
            timeout=10,
            env={**os.environ, "RIBOSOME_LOG": str(log_file)},
    ...
    def test_summary_json_custom_log(self, tmp_path):
        ...
        r = subprocess.run(
            [str(EFFECTORS_DIR / "ribosome"), "summary", "--json", f"--log={log_file}"],
            capture_output=True,
            text=True,
            timeout=10,
            env={**os.environ, "RIBOSOME_LOG": str(tmp_path / "default.jsonl")},
...
# ===================================================================
# ribosome preflight / rate-limit / CLAUDE_CODE_MAX_RETRIES tests
# ===================================================================


class TestRibosomePreflight:
    """Test the preflight health check and rate-limit detection in the ribosome
    ...
    def test_preflight_detects_429(self, tmp_path):
        """When provider returns 429, ribosome should detect it in preflight
    ...
    def test_claude_max_retries_env_set(self, tmp_path):
        ...
        ribosome_src = (EFFECTORS_DIR / "ribosome").read_text()
        assert "CLAUDE_CODE_MAX_RETRIES" in ribosome_src
        assert "RIBOSOME_CLAUDE_RETRIES" in ribosome_src
    ...
    def test_claude_max_retries_default_is_2(self, tmp_path):
        ...
        ribosome_src = (EFFECTORS_DIR / "ribosome").read_text()
        assert "RIBOSOME_CLAUDE_RETRIES:-2" in ribosome_src

    def test_claude_max_retries_overridable(self, tmp_path):
        """RIBOSOME_CLAUDE_RETRIES env var should override the default."""
        ribosome_src = (EFFECTORS_DIR / "ribosome").read_text()
        # The pattern ${RIBOSOME_CLAUDE_RETRIES:-2} allows env override
        assert "${RIBOSOME_CLAUDE_RETRIES:-2}" in ribosome_src
    ...
    def test_timeout_wrapper_present(self):
        ...
        ribosome_src = (EFFECTORS_DIR / "ribosome").read_text()
        assert "timeout" in ribosome_src
        assert "_claude_timeout" in ribosome_src

    def test_empty_output_detected_as_ratelimit(self):
        """The script should detect empty output + non-zero exit as rate-limit."""
        ribosome_src = (EFFECTORS_DIR / "ribosome").read_text()
        assert "${output// /}" in ribosome_src or "output// /" in ribosome_src


class TestRibosomeProviderConfig:
    ...
    def test_volcano_url_is_anthropic_compat(self):
        ...
        ribosome_src = (EFFECTORS_DIR / "ribosome").read_text()
        assert "ark.cn-beijing.volces.com/api/coding" in ribosome_src
    ...
    def test_volcano_uses_api_key_not_auth_token(self):
        ...
        ribosome_src = (EFFECTORS_DIR / "ribosome").read_text()
        assert "unset ANTHROPIC_AUTH_TOKEN" in ribosome_src
        assert 'ANTHROPIC_API_KEY="$_KEY"' in ribosome_src
...
class TestRibosomeVolcanoIntegration:
    """Integration test: verify volcano provider endpoint responds correctly
    to the preflight check pattern used by the ribosome script."""
    ...
    def test_volcano_endpoint_accepts_x_api_key(self):
        ...
        r = subprocess.run(
            [
                ...
                "-d",
                '{"model":"ark-code-latest","max_tokens":1,'
