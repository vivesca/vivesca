...
def test_check_and_heal_unreachable_alerts():
    ...
    mock_status = MagicMock()
    mock_status.return_value = {"reachable": False, "targets": {}}
...
def test_alert_passes_cooldown_to_transport():
    ...
    with patch(
        "metabolon.organelles.secretory_vesicle.secrete_text", return_value="sent"
    ) as mock_secrete:
...
def test_alert_prints_to_stderr():
    ...
    with patch("builtins.print") as mock_print:
        with patch(
            "metabolon.organelles.secretory_vesicle.secrete_text", side_effect=Exception("down")
        ):
