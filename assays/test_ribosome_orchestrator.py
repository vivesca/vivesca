"""Tests for ribosome-orchestrator — Switch between ribosome-daemon, Hatchet, and Temporal backends."""
...
def _load_orchestrator():
    """Load the ribosome-orchestrator module by exec-ing its Python body."""
    source = open(str(Path.home() / "germline/effectors/ribosome-orchestrator")).read()
    # Provide __file__ so Path(__file__) works
    ns: dict = {
        "__name__": "ribosome_orchestrator",
        "__file__": str(Path.home() / "germline/effectors/ribosome-orchestrator"),
    }
    exec(source, ns)
    return ns
...
def test_is_running_hatchet_with_containers():
    ...
    with patch.object(Path, "exists", return_value=False), patch("subprocess.run") as mock_run:
        ...
        mock_run.return_value = subprocess.CompletedProcess(
            args=[],
            returncode=0,
            stdout="hatchet-ribosome-server\nhatchet-ribosome-db\n",
            stderr="",
        )
...
def test_is_running_temporal_with_containers():
    ...
    with patch.object(Path, "exists", return_value=False), patch("subprocess.run") as mock_run:
        mock_run.return_value = subprocess.CompletedProcess(
            args=[], returncode=0, stdout="temporal-ribosome-server\n", stderr=""
        )
...
def test_cmd_stop_daemon(capsys):
    ...
    with (
        patch.dict(_mod, {"_is_running": mock_is_running}),
...
def test_cmd_start_daemon(capsys):
    ...
    with (
        patch.dict(_mod, {"_is_running": lambda b: None}),
...
def test_cmd_switch_stops_all_starts_backend(capsys):
    ...
    with patch.dict(_mod, {"cmd_stop": mock_stop, "cmd_start": mock_start}), patch("time.sleep"):
...
def test_cmd_dispatch_auto_detect_running_backend(capsys):
    ...
    with (
        patch.dict(
            _mod, {"_is_running": lambda b: {"backend": "daemon"} if b == "daemon" else None}
        ),
        patch.dict(_mod, {"_source_env": lambda: {}}),
    ):
...
def test_cmd_dispatch_hatchet():
    ...
    with (
        patch.dict(
            _mod, {"_is_running": lambda b: {"backend": "hatchet"} if b == "hatchet" else None}
        ),
        patch.dict(_mod, {"_source_env": lambda: {}}),
        patch("subprocess.run") as mock_run,
    ):
...
def test_cmd_dispatch_temporal():
    ...
    with (
        patch.dict(
            _mod, {"_is_running": lambda b: {"backend": "temporal"} if b == "temporal" else None}
        ),
        patch.dict(_mod, {"_source_env": lambda: {}}),
        patch("subprocess.run") as mock_run,
    ):
...
def test_main_shows_help_with_h():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "-h"]
...
def test_main_shows_help_with_no_args():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator"]
    mock_sys.exit = MagicMock(side_effect=SystemExit(0))
    with patch.dict(_mod, {"sys": mock_sys}):
        with pytest.raises(SystemExit):
            _mod["main"]()
...
def test_main_status_command():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "status"]
...
def test_main_stop_command():
    ...
    mock_stop = MagicMock()
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "stop"]
...
def test_main_stop_specific_backend():
    ...
    mock_stop = MagicMock()
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "stop", "daemon"]
    with patch.dict(_mod, {"cmd_stop": mock_stop, "sys": mock_sys}):
        _mod["main"]()
...
def test_main_start_requires_backend():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "start"]
...
def test_main_start_invalid_backend():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "start", "invalid"]
    mock_sys.exit = MagicMock(side_effect=SystemExit(1))
    with patch.dict(_mod, {"sys": mock_sys}):
        with pytest.raises(SystemExit):
            _mod["main"]()
...
def test_main_start_valid_backend():
    ...
    mock_start = MagicMock()
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "start", "daemon"]
...
def test_main_switch_requires_backend():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "switch"]
    mock_sys.exit = MagicMock(side_effect=SystemExit(1))
    with patch.dict(_mod, {"sys": mock_sys}):
        with pytest.raises(SystemExit):
            _mod["main"]()
...
def test_main_switch_valid_backend():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "switch", "temporal"]
...
def test_main_dispatch_command():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "dispatch"]
...
def test_main_dispatch_with_backend():
    ...
    mock_dispatch = MagicMock()
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "dispatch", "hatchet"]
    with patch.dict(_mod, {"cmd_dispatch": mock_dispatch, "sys": mock_sys}):
        _mod["main"]()
...
def test_main_unknown_command():
    ...
    mock_sys = MagicMock()
    mock_sys.argv = ["ribosome-orchestrator", "unknown"]
    mock_sys.exit = MagicMock(side_effect=SystemExit(1))
    with patch.dict(_mod, {"sys": mock_sys}):
        with pytest.raises(SystemExit):
            _mod["main"]()
