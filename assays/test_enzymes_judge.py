...
"""Tests for metabolon.enzymes.judge — mock run_cli, verify arg construction."""

import pytest
from unittest.mock import patch, call

# The module imports run_cli from metabolon.organelles.effector
# We mock at the point of use: metabolon.enzymes.judge.run_cli
MODULE = "metabolon.enzymes.judge"
...
def test_enzymes_judge_invalid_rubric_raises():
    ...
    with pytest.raises(ValueError, match="Invalid rubric 'bad'"):
        mod.judge_evaluate(rubric="bad", content="hello")
...
def test_valid_rubrics_accepted():
    ...
    for name in ("article", "job-eval", "outreach"):
        with patch(f"{MODULE}.run_cli", return_value="ok") as mock_cli:
            result = mod.judge_evaluate(rubric=name, content="text")
            assert result == "ok"
...
def test_basic_args():
    ...
    with patch(f"{MODULE}.run_cli", return_value="scored") as mock_cli:
        mod.judge_evaluate(rubric="article", content="some article text")

        binary = mock_cli.call_args[0][0]
        args = mock_cli.call_args[0][1]

        assert "judge" in binary
...
def test_custom_model():
    ...
    with patch(f"{MODULE}.run_cli", return_value="ok") as mock_cli:
        mod.judge_evaluate(rubric="job-eval", content="cv text", model="deepseek")
...
def test_no_context_arg():
    ...
    with patch(f"{MODULE}.run_cli", return_value="ok") as mock_cli:
        mod.judge_evaluate(rubric="article", content="text")
...
def test_content_passed_as_stdin():
    ...
    with patch(f"{MODULE}.run_cli", return_value="ok") as mock_cli:
        mod.judge_evaluate(rubric="article", content="my article content")
...
def test_return_value_passed_through():
    ...
    with patch(f"{MODULE}.run_cli", return_value='{"score": 42}'):
        result = mod.judge_evaluate(rubric="article", content="text")
