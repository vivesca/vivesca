...
class TestDefaultRoutes:
    ...
    def test_returns_copy(self):
        from metabolon.organelles.tissue_routing import default_routes, _DEFAULTS
        r = default_routes()
...
class TestObservedRoutes:
    ...
    def test_falls_back_to_defaults_when_empty(self, mock_bl, mock_mf):
        from metabolon.organelles.tissue_routing import observed_routes, _DEFAULTS
    ...
    @patch("metabolon.organelles.mitophagy.model_fitness", return_value=SAMPLE_FITNESS)
    @patch("metabolon.organelles.mitophagy.is_blacklisted", side_effect=lambda m, t: m == "opus" and t == "repair_novel")
    ...
    @patch("metabolon.organelles.mitophagy.model_fitness", return_value=[
        ...
        {"task_type": "probe", "model": "sonnet", "rate": 0.9, "attempts": 20, "successes": 18},
    ])
    @patch("metabolon.organelles.mitophagy.is_blacklisted", return_value=False)
...
class TestRoute:
    ...
    @patch("metabolon.organelles.tissue_routing.observed_routes", return_value={"probe": "glm"})
    @patch("metabolon.organelles.mitophagy.is_blacklisted", side_effect=Exception("mitophagy down"))
