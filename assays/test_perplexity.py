...
def _make_tmpdir() -> Path:
    """Create a stable tmpdir via tempfile.mkdtemp (avoids pytest tmp_path races)."""
    return Path(tempfile.mkdtemp(prefix="perplexity_test_"))
...
def _make_fake_curl(tmpdir: Path, response_body: str) -> Path:
    ...
    are needed (avoids tmp cleanup races between pytest and subprocess).
    """
    fake = tmpdir / "curl"
...
def _run_with_fake_curl(
    ...
    Uses its own tempfile.mkdtemp — immune to pytest tmp_path cleanup races.
    """
    tmpdir = _make_tmpdir()
...
class TestModeMapping:
    ...
    def test_mode_maps_to_correct_model(self, mode, expected_model):
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, request = _run_with_fake_curl(mode, "test query", body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
        assert request.get("model") == expected_model

    def test_query_passed_through(self):
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, request = _run_with_fake_curl("search", "what is life?", body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
...
class TestResponseParsing:
    def test_extracts_content_from_choices(self):
        content = "Paris is the capital of France."
        body = json.dumps({"choices": [{"message": {"content": content}}]})
        proc, _ = _run_with_fake_curl("search", "capital of france", body)
    ...
    def test_multiline_content(self):
        content = "Line 1\nLine 2\nLine 3"
        body = json.dumps({"choices": [{"message": {"content": content}}]})
        proc, _ = _run_with_fake_curl("ask", "multi", body)
        assert proc.returncode == 0
        assert "Line 1" in proc.stdout
        assert "Line 3" in proc.stdout

    def test_error_response_exits_nonzero(self):
        body = json.dumps({"error": {"message": "Rate limit exceeded"}})
        proc, _ = _run_with_fake_curl("search", "test", body)
        assert proc.returncode != 0
        assert "Rate limit" in proc.stderr

    def test_unexpected_json_dumped_as_is(self):
    ...
    def test_non_json_response_passthrough(self):
        ...
        proc, _ = _run_with_fake_curl("reason", "test", body)
        assert proc.returncode == 0
...
class TestQueryEscaping:
    def test_query_with_quotes_escaped(self):
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, request = _run_with_fake_curl("search", 'He said "hello"', body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
        assert request["messages"][0]["content"] == 'He said "hello"'

    def test_query_with_special_chars(self):
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, request = _run_with_fake_curl("search", "price: $5 & <tag>", body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
        assert request["messages"][0]["content"] == "price: $5 & <tag>"

    def test_query_with_single_quotes(self):
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, request = _run_with_fake_curl("search", "it's a test", body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
        assert request["messages"][0]["content"] == "it's a test"

    def test_query_with_unicode(self):
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, request = _run_with_fake_curl("ask", "Héllo wörld 日本語", body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
...
class TestExtraArgs:
    def test_three_args_ignores_third(self):
        """Extra positional args are ignored by bash — only $1 and $2 are used."""
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, _ = _run_with_fake_curl("search", "hello", body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
...
class TestAuthHeader:
    def test_curl_receives_bearer_token(self):
...
class TestPayloadStructure:
    def test_request_is_valid_json(self):
        ...
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, request = _run_with_fake_curl("search", "test", body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
    ...
    def test_long_query_handled(self):
        ...
        long_query = "x" * 10000
        body = json.dumps({"choices": [{"message": {"content": "ok"}}]})
        proc, request = _run_with_fake_curl("search", long_query, body)
        assert proc.returncode == 0, f"stderr: {proc.stderr}"
