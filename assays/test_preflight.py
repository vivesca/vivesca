...
def test_preflight_result_add_critical():
    from metabolon.metabolism.preflight import CheckResult, PreflightResult
...
# --- check_ribosome_ready tests ---


def test_check_ribosome_binary_found_in_path():
    from metabolon.metabolism.preflight import check_ribosome_binary

    with patch("metabolon.metabolism.preflight.shutil.which") as mock_which:
        mock_which.return_value = "/usr/local/bin/ribosome"
        result = check_ribosome_binary()
        assert result.passed
        assert "Found" in result.message


def test_check_ribosome_binary_found_in_default():
    from metabolon.metabolism.preflight import check_ribosome_binary

    with patch("metabolon.metabolism.preflight.shutil.which") as mock_which:
        mock_which.return_value = None
        with patch("metabolon.metabolism.preflight.HOME") as mock_home:
            mock_home.__truediv__ = lambda self, x: Path("/home/user/germline/effectors/ribosome")
...
def test_check_ribosome_binary_not_found():
    from metabolon.metabolism.preflight import check_ribosome_binary
...
def test_check_ribosome_api_key_valid_provider():
    from metabolon.metabolism.preflight import check_ribosome_api_key

    with patch.dict("os.environ", {"ZHIPU_API_KEY": "test-key-12345678"}):
        result = check_ribosome_api_key("zhipu")
        assert result.passed
...
def test_check_ribosome_api_key_unknown_provider():
    ...
    result = check_ribosome_api_key("unknown_provider")
    assert not result.passed
    assert "Unknown provider" in result.message


def test_check_ribosome_api_key_missing():
    from metabolon.metabolism.preflight import check_ribosome_api_key
...
def test_check_ribosome_log_freshness_missing():
    from metabolon.metabolism.preflight import check_ribosome_log_freshness

    with patch("metabolon.metabolism.preflight.RIBOSOME_LOG") as mock_log:
...
def test_check_ribosome_log_freshness_recent():
    from metabolon.metabolism.preflight import check_ribosome_log_freshness
...
def test_check_ribosome_log_freshness_stale():
    from metabolon.metabolism.preflight import check_ribosome_log_freshness
...
def test_check_ribosome_ready_comprehensive():
    from metabolon.metabolism.preflight import check_ribosome_ready

    with patch("metabolon.metabolism.preflight.check_ribosome_binary") as mock_binary:
        with patch("metabolon.metabolism.preflight.check_ribosome_api_key") as mock_key:
            with patch("metabolon.metabolism.preflight.check_provider_health") as mock_health:
                with patch(
                    "metabolon.metabolism.preflight.check_ribosome_log_freshness"
                ) as mock_log:
                    mock_binary.return_value = MagicMock(passed=True, name="ribosome_binary")
                    mock_key.return_value = MagicMock(passed=True, name="ribosome_api_key")
                    mock_health.return_value = MagicMock(passed=True, name="provider_health")
                    mock_log.return_value = MagicMock(passed=True, name="ribosome_log")
                    result = check_ribosome_ready(provider="zhipu")
        ...
        with patch("metabolon.metabolism.preflight.check_ribosome_api_key") as mock_key:
            with patch("metabolon.metabolism.preflight.check_provider_health") as mock_health:
                with patch(
                    ...
                    result = check_ribosome_ready(provider="zhipu", skip_health_check=True)
