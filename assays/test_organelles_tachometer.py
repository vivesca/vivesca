...
class TestSlowestRecent:
    ...
    def test_returns_all_fields(self, mock_rl):
        ...
        entries = [
            _entry(
                ts=NOW - timedelta(minutes=5), duration_s=50.0, plan="p", tool="glm", success=False
            )
        ]
        mock_rl.return_value = entries
        with patch("metabolon.organelles.tachometer.datetime") as mock_dt:
...
class TestCoachingEffectiveness:
    ...
    def test_improvement(self, mock_rl, mock_cn):
        ...
        before = [
            _entry(ts=coaching_ts - timedelta(minutes=i), success=(i % 2 == 0))
            for i in range(1, 11)
        ]
        after = [_entry(ts=coaching_ts + timedelta(minutes=i), success=True) for i in range(10)]
    ...
    def test_regression(self, mock_rl, mock_cn):
        ...
        coaching_ts = NOW - timedelta(hours=2)
        before = [
            _entry(ts=coaching_ts - timedelta(minutes=i), success=True) for i in range(1, 11)
        ]
        after = [_entry(ts=coaching_ts + timedelta(minutes=i), success=False) for i in range(10)]
        mock_rl.return_value = before + after
