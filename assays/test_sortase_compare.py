...
def test_sortase_compare_delta_str_increase():
    result = _delta_str(50.0, 75.0)
    assert "↑" in result
    assert "50%" in result


def test_sortase_compare_delta_str_decrease():
    result = _delta_str(100.0, 80.0)
    assert "↓" in result
    assert "20%" in result


def test_sortase_compare_delta_str_zero_base():
    assert _delta_str(0, 10) == ""


def test_sortase_compare_delta_str_no_change():
