...
class TestTaste:
    @pytest.mark.asyncio
    @patch("metabolon.metabolism.gates.transduce", return_value="PASS")
    def test_pass(self, mock_transduce):
        result = asyncio.get_event_loop().run_until_complete(
            taste("my_tool", "original desc", "new desc")
        )
        assert result.passed is True
        assert "PASS" in result.reason
        mock_transduce.assert_called_once()
        # first arg is model name "haiku"
        assert mock_transduce.call_args[0][0] == "haiku"

    @pytest.mark.asyncio
    @patch("metabolon.metabolism.gates.transduce", return_value="FAIL: too vague")
    def test_fail_with_reason(self, mock_transduce):
        result = asyncio.get_event_loop().run_until_complete(
            taste("my_tool", "original desc", "vague desc")
        )
        assert result.passed is False
        assert "too vague" in result.reason

    @pytest.mark.asyncio
    @patch("metabolon.metabolism.gates.transduce", return_value="FAIL:")
    def test_fail_empty_reason(self, mock_transduce):
        result = asyncio.get_event_loop().run_until_complete(
            taste("my_tool", "original", "variant")
        )
        assert result.passed is False
    ...
    @pytest.mark.asyncio
    @patch("metabolon.metabolism.gates.transduce", return_value="PASS\n")
    def test_pass_with_trailing_whitespace(self, mock_transduce):
        result = asyncio.get_event_loop().run_until_complete(
            taste("tool", "orig", "variant")
        )
        assert result.passed is True

    @pytest.mark.asyncio
    @patch(
        "metabolon.metabolism.gates.transduce",
        return_value="FAIL: description is misleading",
    )
    def test_prompt_contains_tool_info(self, mock_transduce):
        asyncio.get_event_loop().run_until_complete(
            taste("search_tool", "finds things", "loses things")
        )
