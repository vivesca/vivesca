...
"""Tests for metabolon.enzymes.thrombin — emergency process stabilization."""
...
class TestPsAction:
    @patch("metabolon.enzymes.thrombin.subprocess.run")
...
class TestKillAction:
    ...
    def test_kill_success(self, mock_run):
        ...
        result = thrombin(action="kill", pattern="rogue_proc")
    ...
    def test_kill_no_match(self, mock_run):
        ...
        result = thrombin(action="kill", pattern="nothing")
    ...
    def test_kill_error_exit(self, mock_run):
        ...
        result = thrombin(action="kill", pattern="foo")
    ...
    def test_kill_invalid_signal(self):
        result = thrombin(action="kill", pattern="x", signal="BOGUS")
    ...
    def test_kill_custom_signal(self, mock_run):
        mock_run.return_value = _make_completed(returncode=0)
        result = thrombin(action="kill", pattern="x", signal="KILL")
...
class TestHandoffAction:
    @patch(
        "metabolon.enzymes.thrombin._HANDOFF_DIR",
        new_callable=lambda: lambda: Path("/tmp/thrombin_test_handoff"),
    )
    def test_handoff_writes_note(self, tmp_path_factory):
        ...
        with patch("metabolon.enzymes.thrombin._HANDOFF_DIR", handoff_dir):
            result = thrombin(
                action="handoff",
...
class TestUnknownAction:
    def test_unknown_action_returns_error(self):
        result = thrombin(action="explode")
    ...
    def test_action_case_insensitive(self):
        ...
        with patch("metabolon.enzymes.thrombin.subprocess.run") as mock_run:
            mock_run.return_value = _make_completed(stdout="")
