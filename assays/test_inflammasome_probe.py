...
inflammasome_probe_code = Path(
    str(Path.home() / "germline/effectors/inflammasome-probe")
).read_text()
...
def test_load_cooldown_valid_file():
    ...
    mock_data = {"probe1": "2026-03-30T10:00:00", "probe2": "2026-03-31T10:00:00"}
...
def test_is_novel_failure_import_error_returns_true():
    ...
    with patch(
        "metabolon.metabolism.infection.recall_infections", side_effect=ImportError("no module")
    ):
...
def test_novel_failure_repair_skips_rheotaxis_not_set():
    ...
    results = [
        {"name": "rheotaxis", "passed": False, "repair_attempted": None, "message": "env not set"}
    ]
...
def test_novel_failure_repair_respects_max_repairs_cap():
    ...
    def mock_import(name, *args, **kwargs):
        if name == "metabolon.organelles.crispr" or name.startswith(
        ...
        with patch(f"{current_module.__name__}._is_novel_failure", return_value=True):
            with patch(f"{current_module.__name__}._is_on_cooldown", return_value=False):
                with patch("builtins.__import__", side_effect=mock_import):
                    with patch(
                        f"{current_module.__name__}._dispatch_diagnosis",
                        return_value="FIXED: test",
                    ):
...
def test_novel_failure_repair_handles_fixed_result():
    ...
    results = [
        {"name": "probe1", "passed": False, "repair_attempted": None, "message": "test failure"}
    ]
        ...
        with patch(f"{current_module.__name__}._is_novel_failure", return_value=True):
            with patch(f"{current_module.__name__}._is_on_cooldown", return_value=False):
                with patch("builtins.__import__", side_effect=mock_import):
                    with patch(
                        f"{current_module.__name__}._dispatch_diagnosis",
                        return_value="FIXED: fixed the issue",
                    ):
                        with patch(f"{current_module.__name__}._save_cooldown"):
                            with patch(
                                f"{current_module.__name__}._log_methylation_candidate"
                            ) as mock_log:
                                novel_failure_repair(results, "test")
                                assert results[0]["repair_attempted"] == "agent:fixed"
        ...
        with patch(f"{current_module.__name__}._is_novel_failure", return_value=True):
            with patch(f"{current_module.__name__}._is_on_cooldown", return_value=False):
                with patch("builtins.__import__", side_effect=mock_import):
                    with patch(
                        f"{current_module.__name__}._dispatch_diagnosis",
                        return_value="UNFIXED: could not reproduce",
                    ):
                        with patch(f"{current_module.__name__}._save_cooldown"):
                            novel_failure_repair(results, "test")
