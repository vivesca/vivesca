"""Tests for ribosome wall-limit feature (t-timeout1).

The ribosome bash script should respect RIBOSOME_WALL_LIMIT env var to avoid
...
RIBOSOME_SCRIPT = str(Path.home() / "germline" / "effectors" / "ribosome")


def _run_ribosome_snippet(env_override: dict, timeout: int = 15) -> subprocess.CompletedProcess:
    """Run ribosome with a fake provider that always 429s, to trigger retry logic."""
    env = {**os.environ, **env_override}
    # Use a nonexistent API endpoint so preflight fails fast with rate-limit
    env["RIBOSOME_PROVIDER"] = "zhipu"
    env["ANTHROPIC_API_KEY"] = "fake-key-for-test"
    env["ANTHROPIC_BASE_URL"] = "http://127.0.0.1:1"  # connection refused = fast fail
    return subprocess.run(
        ["bash", RIBOSOME_SCRIPT, "--provider", "zhipu", "--max-turns", "1", "echo test"],
        capture_output=True,
        text=True,
        timeout=timeout,
        env=env,
...
class TestWallLimitSkipsRetry:
    """When wall-limit is very short, ribosome should not retry after first failure."""

    def test_wall_limit_exits_without_full_retries(self):
        """With RIBOSOME_WALL_LIMIT=5 (5 seconds), ribosome should not sleep 30s for backoff."""
        start = time.monotonic()
        result = _run_ribosome_snippet({"RIBOSOME_WALL_LIMIT": "5"}, timeout=30)
        ...
        assert elapsed < 20, (
            f"Ribosome took {elapsed:.0f}s — wall-limit did not prevent retry backoff"
        )
...
class TestWallLimitAllowsFirstAttempt:
    ...
    def test_first_attempt_runs_with_large_limit(self):
        ...
        result = _run_ribosome_snippet({"RIBOSOME_WALL_LIMIT": "9999"}, timeout=30)
...
class TestWallLimitDefault:
    """When RIBOSOME_WALL_LIMIT is unset, default should be 1680 (28 min)."""

    def test_default_value_is_1680(self):
        """Parse the ribosome script source to verify the default."""
