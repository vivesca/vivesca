...
"""Tests for endocytosis-analyze — article classification and ranking effector."""
...
def _load_module():
    """Load endocytosis-analyze by exec-ing its source."""
    source = open(str(Path.home() / "germline/effectors/endocytosis-analyze")).read()
    ns: dict = {"__name__": "endocytosis_analyze"}
    exec(source, ns)
    return ns
...
def tmp_article_dir(tmp_path):
    ...
    (tmp_path / "bank_ai.json").write_text(
        json.dumps(
            {
    ...
    (tmp_path / "pure_ai.json").write_text(
        json.dumps(
            {
    ...
    (tmp_path / "regulation.json").write_text(
        json.dumps(
            {
    ...
    (tmp_path / "2026-03-01_notes.md").write_text(
        textwrap.dedent("""\
...
class TestLoadArticles:
    ...
    def test_md_extracts_date_from_filename(self, tmp_article_dir):
        articles = load_articles(tmp_article_dir)
        md = next(a for a in articles if a["file"] == "2026-03-01_notes.md")
...
class TestFormatSummaryTable:
    def test_contains_headers(self, classified_articles):
        ...
        assert "ENDOCYTOSIS ARTICLE ANALYSIS SUMMARY" in output
...
class TestCLI:
    def test_help_flag(self):
        result = subprocess.run(
            ["python3", str(Path.home() / "germline/effectors/endocytosis-analyze"), "--help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
    ...
    def test_cli_with_dir(self, tmp_article_dir):
        result = subprocess.run(
            [
                "python3",
                str(Path.home() / "germline/effectors/endocytosis-analyze"),
                "--dir",
                str(tmp_article_dir),
                "--top",
                "3",
        ...
        assert result.returncode == 0
        assert "ENDOCYTOSIS ARTICLE ANALYSIS SUMMARY" in result.stdout
    ...
    def test_cli_output_file(self, tmp_article_dir, tmp_path):
        ...
        assert "ENDOCYTOSIS ARTICLE ANALYSIS SUMMARY" in content
