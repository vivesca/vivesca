...
from metabolon.lysin.fetch import fetch_summary, search_term, fetch_sections, BioArticle, _strip_html
...
def test_search_term(mock_get):
    ...
    mock_response.json.return_value = ["endocytosis", ["Endocytosis", "Endocytosis (virus)"], [], []]
...
def test_fetch_summary_exact_match(mock_get):
    ...
    assert article.mechanism == "Endocytosis is a cellular process. It involves cells taking in substances."
...
def test_fetch_sections(mock_get):
    ...
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "lead": {"sections": [{"id": 0, "text": "<p>Lead text</p>"}]}, # Typically no line in lead, should be skipped or parsed
        "remaining": {"sections": [{"id": 1, "line": "<b>Types</b>", "text": "<p>Some types</p>"}]}
    }
    mock_get.return_value = mock_response
