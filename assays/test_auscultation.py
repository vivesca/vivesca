...
class TestGlobLogs:
    ...
    def test_returns_empty_when_dirs_missing(self, tmp_path):
        ...
        with (
            patch("metabolon.enzymes.auscultation._LOG_DIR", missing),
...
class TestAuscultationLogs:
    ...
    def test_logs_with_filter_pattern(self, tmp_path):
        log_dir = self._make_logs(
            tmp_path, {"app.log": "ERROR something broke\nINFO all good\nWARN hmm\n"}
        )
        with (
            patch("metabolon.enzymes.auscultation._LOG_DIR", log_dir),
            patch("metabolon.enzymes.auscultation._TMP_DIR", tmp_path / "nope"),
...
class TestAuscultationErrors:
    ...
    def test_errors_without_number_normalization(self, tmp_path):
        log_dir = self._make_logs(
            tmp_path, {"app.log": "ERROR code 42\nERROR code 42\nERROR code 99\n"}
        )
        with (
            patch("metabolon.enzymes.auscultation._LOG_DIR", log_dir),
            patch("metabolon.enzymes.auscultation._TMP_DIR", tmp_path / "nope"),
    ...
    def test_errors_custom_severity(self, tmp_path):
        log_dir = self._make_logs(tmp_path, {"app.log": "FATAL crash\nERROR bug\nINFO ok\n"})
        with (
            patch("metabolon.enzymes.auscultation._LOG_DIR", log_dir),
            patch("metabolon.enzymes.auscultation._TMP_DIR", tmp_path / "nope"),
        ):
