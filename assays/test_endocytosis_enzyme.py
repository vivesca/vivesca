...
class TestStatusResult:
    ...
    @patch("metabolon.enzymes.endocytosis.restore_state")
    @patch("metabolon.enzymes.endocytosis.restore_config")
    def test_basic_status_output(
        self, mock_restore_config, mock_restore_state, mock_dt
    ):
        ...
        with (
            patch("metabolon.enzymes.endocytosis._file_age", return_value="2h old"),
    ...
    def test_status_with_state(
        ...
        with (
            patch("metabolon.enzymes.endocytosis._file_age", return_value="ok"),
            patch.object(Path, "exists", return_value=False),
        ):
            result = _status_result()
    ...
    def test_status_with_article_cache(
        self, mock_restore_config, mock_restore_state, mock_dt, tmp_path
    ):
        ...
        with patch("metabolon.enzymes.endocytosis._file_age", return_value="ok"):
            result = _status_result()
...
class TestStatsResult:
    ...
    @patch("metabolon.enzymes.endocytosis.affinity_stats")
    @patch("metabolon.enzymes.endocytosis._read_jsonl")
...
class TestTopResult:
    ...
    @patch("metabolon.enzymes.endocytosis.top_cargo")
    ...
    def test_with_items(self, mock_top):
        ...
        assert "Banking angle" not in result.output.split("Second")[1].split("\n")[0]
...
class TestEndocytosisDispatch:
    ...
    def test_fetch_action(self):
        ...
        with (
            patch("metabolon.enzymes.endocytosis.restore_config", return_value=mock_config),
            patch("metabolon.enzymes.endocytosis.lockfile", return_value=mock_lock),
            patch("metabolon.enzymes.endocytosis._fetch_locked") as mock_fetch,
        ):
