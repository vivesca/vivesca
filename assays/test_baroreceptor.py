...
class TestBuildWeatherLine:
    ...
    def _make_fnd(self, lo=22, hi=28, desc="Cloudy."):
    ...
    def test_basic_output(self):
        ...
        line = build_weather_line(self._make_now(), self._make_fnd(), {})
        assert "22" in line
        assert "28" in line
    ...
    def test_rain_included(self):
        from metabolon.organelles.baroreceptor import build_weather_line
        line = build_weather_line(self._make_now(rain=3.5), self._make_fnd(), {})
        assert "3.5mm" in line

    def test_high_uv(self):
        from metabolon.organelles.baroreceptor import build_weather_line
        line = build_weather_line(self._make_now(uv=8), self._make_fnd(), {})
        assert "UV 8" in line

    def test_muggy(self):
        from metabolon.organelles.baroreceptor import build_weather_line
        line = build_weather_line(self._make_now(humidity=95), self._make_fnd(), {})
        assert "muggy" in line

    def test_warnings_shown(self):
        from metabolon.organelles.baroreceptor import build_weather_line
        warn = {"WRAIN": {"name": "Rainstorm Warning Signal"}}
        line = build_weather_line(self._make_now(), self._make_fnd(), warn)
    ...
    def test_sunny_emoji(self):
        from metabolon.organelles.baroreceptor import build_weather_line
        line = build_weather_line(self._make_now(), self._make_fnd(desc="Fine."), {})
