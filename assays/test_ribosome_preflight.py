"""Tests for ribosome preflight health check and rate-limit retry logic.
...
def _run_ribosome_fragment(
    env_extra: dict | None = None, args: list[str] | None = None
) -> subprocess.CompletedProcess:
    """Run the ribosome script with given args and return the result."""
    env = os.environ.copy()
...
def test_ribosome_help():
    """ribosome --help exits with 0 and shows usage."""
    r = _run_ribosome_fragment(args=["--help"])
    assert r.returncode == 0
    assert "ribosome" in r.stdout.lower()


def test_ribosome_unknown_provider():
    """ribosome --provider nonexistent exits non-zero."""
    r = _run_ribosome_fragment(args=["--provider", "nonexistent", "test task"])
    assert r.returncode != 0
    assert "Unknown provider" in r.stderr or "Unknown provider" in r.stdout


def test_ribosome_summary_no_log():
    """ribosome summary with a temp log dir exits with 1 (no log file)."""
    with tempfile.TemporaryDirectory() as tmp:
        env = {"RIBOSOME_LOG": os.path.join(tmp, "nonexistent.jsonl")}
        r = _run_ribosome_fragment(env_extra=env, args=["summary", "--quiet"])
...
def test_summary_with_existing_log():
    """ribosome summary reads and parses JSONL entries."""
    with tempfile.TemporaryDirectory() as tmp:
        log_file = Path(tmp) / "ribosome.jsonl"
        log_file.write_text(
        ...
        r = _run_ribosome_fragment(env_extra=env, args=["summary"])
        assert r.returncode == 0
...
def test_summary_json_flag():
    """ribosome summary --json outputs valid JSON."""
    with tempfile.TemporaryDirectory() as tmp:
        log_file = Path(tmp) / "ribosome.jsonl"
        log_file.write_text(
        ...
        env = {"RIBOSOME_LOG": str(log_file)}
        r = _run_ribosome_fragment(env_extra=env, args=["summary", "--json"])
        assert r.returncode == 0
...
def test_volcano_preflight_returns_429():
    ...
    r = subprocess.run(
        [
            ...
            "-d",
            '{"model":"ark-code-latest","max_tokens":1,"messages":[{"role":"user","content":"ping"}]}',
            "--connect-timeout",
            "10",
            "--max-time",
            "15",
