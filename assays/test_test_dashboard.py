...
class TestDashboardOutput:
    ...
    def test_script_runs_successfully(self):
        ...
        result = subprocess.run(
            [str(DASHBOARD_SCRIPT)], capture_output=True, text=True, timeout=120
        )
        assert result.returncode == 0
        assert "Total tests:" in result.stdout
        assert "Pass rate:" in result.stdout
        assert "Ribosome runs by provider:" in result.stdout
    ...
    def test_outputs_provider_counts(self):
        ...
        assert (
            "infini:" in result.stdout
            or "volcano:" in result.stdout
...
class TestRibosomeLogParsing:
    """Unit tests for ribosome.jsonl parsing logic."""
...
class TestPassRateCalculation:
    ...
    def test_nonzero_exit_codes(self):
        ...
        entries = [{"exit": 0}, {"exit": 1}, {"exit": 2}, {"exit": None}]
        passed = sum(1 for e in entries if e.get("exit") == 0)
        rate = passed / len(entries) if entries else 0.0
        assert rate == 0.25
