...
def test_log_resources_ribosome_count(tmp_path):
    jsonl = tmp_path / "soma-resources.jsonl"
    with (
        _P("METRICS_PATH", jsonl),
        patch("shutil.disk_usage", return_value=MagicMock(used=50, total=100, free=50)),
        ...
        log_resources()
    data = json.loads(jsonl.read_text().strip())
    assert data["ribosome_count"] == 5


def test_log_resources_ribosome_count_zero_on_fail(tmp_path):
    jsonl = tmp_path / "soma-resources.jsonl"
    with (
        _P("METRICS_PATH", jsonl),
        patch("shutil.disk_usage", return_value=MagicMock(used=50, total=100, free=50)),
...
def test_alert_no_crit_no_call(tmp_path):
    ...
    with _P("ALERT_STAMP", stamp), patch("subprocess.run") as mr:
...
def test_alert_creates_stamp_dir(tmp_path):
    ...
    health = {"disk_volume": {"status": "crit", "free_gb": 0.5}}
    with _P("ALERT_STAMP", stamp), patch("subprocess.run"):
        alert_on_crit(health)
    assert stamp.exists()
...
def test_main_calls_log_resources_and_alert(tmp_path):
    ...
    with (
        ...
        patch.object(_mod["sys"], "argv", ["w"]),
        patch.object(_mod["time"], "sleep", side_effect=KeyboardInterrupt),
    ):
        main()
