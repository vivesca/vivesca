...
def test_censor_evaluate_invalid_rubric():
    """Test that ValueError is raised for invalid rubrics."""
    with pytest.raises(ValueError) as excinfo:
        censor_evaluate("invalid_rubric", "content")
...
def test_censor_evaluate_valid_rubrics_no_context(rubric):
    """Test that valid rubrics work without context."""
    mock_return = '{"score": 90, "feedback": "Great"}'
    with patch("metabolon.enzymes.censor.run_cli") as mock_run:
        mock_run.return_value = mock_return
        result = censor_evaluate(rubric, "test content")
