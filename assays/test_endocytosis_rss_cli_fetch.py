...
def test_fetch_fallback_and_zeros(monkeypatch, mock_cfg, capsys):
    ...
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.fetcher.internalize_rss",
        lambda _url, _since, **kwargs: None,
    )
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.fetcher.internalize_web",
        lambda _url, **kwargs: [{"title": "Web Article", "link": "https://live.web/1"}],
    )

    # Mock other needed functions
    # Patch both the module attribute and the name bound in cli's namespace
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.state.refractory_elapsed",
        lambda *args, **kwargs: True,
    )
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.cli.refractory_elapsed", lambda *args, **kwargs: True
    )
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.relevance.receptor_signal_ratio",
    ...
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.log.recall_title_prefixes", lambda _p: set()
    )
    monkeypatch.setattr("metabolon.organelles.endocytosis_rss.log.is_noise", lambda _t: False)
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.log.serialize_markdown", lambda *args: "# News"
    )
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.log.record_cargo", lambda *args: None
    )
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.fetcher.archive_cargo", lambda *args: None
    )
    ...
    monkeypatch.setattr(
        "metabolon.organelles.endocytosis_rss.fetcher.internalize_web", lambda _url, **kwargs: []
    )
