...
def _infection(
    tool="t", fingerprint="fp", count=5, last_error="err", last_seen="2025-01-01", healed_count=0
):
    return dict(
        tool=tool,
...
class TestRepair:
    ...
    def test_accepted_repair(self, mock_trans, mock_immune):
        ...
        mock_immune.return_value = ImmuneResult(
            candidate="fixed",
            accepted=True,
            gate_result=GateResult(True, "ok"),
            attempts=1,
        )
    ...
    def test_rejected_repair(self, mock_trans, mock_immune):
        ...
        mock_immune.return_value = ImmuneResult(
            candidate=None,
            accepted=False,
            gate_result=GateResult(False, "too short"),
            attempts=3,
        )
        r = repair(_state(infections=[_infection()]))
        rec = r["repairs_attempted"][0]
...
class TestVerifyFix:
    ...
    def test_partial_heal_keeps_remaining(self):
        state = _state(
            health_score=0.4,
            infections=[_infection(fingerprint="fp1"), _infection(fingerprint="fp2")],
            repairs_attempted=[
                dict(
                    tool="t",
                    fingerprint="fp1",
...
class TestSweepForIssues:
    ...
    @patch("metabolon.organelles.metabolism_loop.transduce_safe")
    @patch("metabolon.metabolism.signals.DEFAULT_LOG")
    def test_no_stimuli_no_infections_empty(
        self, mock_log, mock_trans, mock_select, mock_sense, mock_summary
    ):
        mock_log.exists.return_value = False
        r = sweep_for_issues(_state())
    ...
    def test_sweep_result_fields(self, mock_log, mock_trans, mock_select, mock_sense):
        ...
        mock_log.read_text.return_value = s.model_dump_json() + "\n"
        mock_sense.return_value = {
            "slow": _emotion("slow", valence=0.1, activations=5, success_rate=0.3)
        }
    ...
    def test_report_with_infections(self):
        r = report(
            _state(
                ...
                infections=[
                    _infection(
                        tool="bad_tool", fingerprint="fp1", count=7, last_error="timeout after 30s"
                    )
                ],
            )
        )
...
class TestRouting:
    ...
    def test_route_after_verify_still_infected_under_cap(self):
        assert route_after_verify(_state(_has_infections=True, iteration=1)) == "measure_fitness"

    def test_route_after_verify_still_infected_at_cap(self):
        assert (
            route_after_verify(_state(_has_infections=True, iteration=MAX_ITERATIONS))
            == "sweep_for_issues"
        )

    def test_route_after_verify_healed(self):
        assert route_after_verify(_state(_has_infections=False, iteration=1)) == "sweep_for_issues"

    def test_route_after_sweep_issues_under_cap(self):
        assert (
            route_after_sweep(_state(_sweep_found_issues=True, iteration=1)) == "measure_fitness"
        )

    def test_route_after_sweep_issues_at_cap(self):
        assert (
            route_after_sweep(_state(_sweep_found_issues=True, iteration=MAX_ITERATIONS))
            == "report"
        )

    def test_route_after_sweep_no_issues(self):
        assert route_after_sweep(_state(_sweep_found_issues=False, iteration=1)) == "report"
...
class TestRunMetabolism:
    ...
    def test_infected_repair_rejected_stays_unresolved(
        ...
        mock_immune.return_value = ImmuneResult(
            candidate=None,
            accepted=False,
            gate_result=GateResult(False, "gate rejected"),
            attempts=3,
        )
        mock_select.return_value = []
        mock_summary.return_value = ""
