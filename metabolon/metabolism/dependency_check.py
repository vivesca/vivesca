...
def check_env_var(name: str, min_length: int = 10) -> DependencyStatus:
    ...
    if len(val) < min_length:
        return DependencyStatus(name=name, healthy=False, message=f"Too short ({len(val)} chars)", category="api_key")
...
def check_binary(name: str) -> DependencyStatus:
    ...
    if not path:
        return DependencyStatus(name=name, healthy=False, message="Not found on PATH", category="binary")
    try:
...
def check_git_repo(path: Path, name: str) -> DependencyStatus:
    ...
    if not path.exists():
        return DependencyStatus(name=name, healthy=False, message="Path missing", category="git_repo")
    git_dir = path / ".git"
    if not git_dir.exists():
        return DependencyStatus(name=name, healthy=False, message="Not a git repo", category="git_repo")
    try:
        ...
        if result.returncode != 0:
            return DependencyStatus(name=name, healthy=False, message="git rev-parse failed", category="git_repo")
