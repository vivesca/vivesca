...
Exposes the listening surfaces the auscultation skill taps:
  auscultation_logs    — tail and grep vivesca LaunchAgent logs
  auscultation_errors  — count and rank error patterns across all logs
"""
...
@tool(
    name="auscultation_logs",
    description="Tail vivesca logs. Returns last N lines, optionally filtered.",
    annotations=ToolAnnotations(readOnlyHint=True, destructiveHint=False),
)
def auscultation_logs(
    filter_pattern: str = "",
    tail_lines: int = 100,
    log_name: str = "",
) -> str:
...
@tool(
    name="auscultation_errors",
    description="Count and rank error patterns across vivesca logs. Surfaces frequencies.",
    annotations=ToolAnnotations(readOnlyHint=True, destructiveHint=False),
)
def auscultation_errors(
    severity: str = "ERROR|WARN|FAIL",
    top_n: int = 20,
    normalize_numbers: bool = True,
) -> str:
    """Aggregate error patterns across all vivesca logs by frequency.

    One error = event. Same error N times = pattern. Pattern = finding.
