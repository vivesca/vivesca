...
def _read_efferens() -> str:
    try:
        ...
        messages = acta.read()  # type: ignore
    except Exception as exc:
...
def _entrainment_brief() -> PinocytosisResult:
    ...
    if health:
        warning_lines = [line for line in health.splitlines() if "warning" in line.lower() or "red" in line.lower()]
        if warning_lines:
    ...
    if flywheel:
        miss_lines = [line for line in flywheel.splitlines() if "miss" in line.lower() or "0%" in line]
        if miss_lines:
    ...
    try:
        ...
        if "NEEDS_ATTENTION" in kinesin_output or "CRITICAL" in kinesin_output:
            attention = [line for line in kinesin_output.splitlines() if "NEEDS_ATTENTION" in line or "CRITICAL" in line]
...
def _overnight_results(task: str) -> PinocytosisResult:
    ...
    return PinocytosisResult(
        output=invoke_organelle(str(VIVESCA_ROOT / "effectors" / "overnight-gather"), args, timeout=15)
    )
...
def _overnight_list() -> PinocytosisResult:
    ...
    return PinocytosisResult(
        output=invoke_organelle(str(VIVESCA_ROOT / "effectors" / "overnight-gather"), ["list"], timeout=15)
    )
...
def pinocytosis(
    ...
    if action == "morning":
        ...
        return PinocytosisResult(output=photoreception.intake(as_json=json_output, send_weather=send_weather))
    if action == "day":
