...
def brainstorm(state: PolarizationState) -> dict:
    ...
    consumption_signal = "Produce more." if consumption <= 3 else (
        "Self-sufficient outputs only." if consumption <= 8 else
        "Overproduction. Triage only."
    )
...
def build_graph() -> StateGraph:
    ...
    graph.add_conditional_edges("stopping_gate", should_continue, {
        "preflight": "preflight",
        "report": "report",
    })
