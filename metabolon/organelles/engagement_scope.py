...
_REGULATORY_KEYWORDS = [
    "HKMA", "SFC", "MAS", "BIS", "Basel", "BCBS", "GDPR", "PDPO",
    "SOX", "PCI-DSS", "ISO 27001", "DORA", "NIS2", "CPS 234",
    "model risk", "SR 11-7", "SS1/23", "AI governance", "explainability",
]
...
_SKILL_KEYWORDS = [
    "python", "java", "cloud", "aws", "azure", "gcp", "kubernetes", "docker",
    "machine learning", "AI", "NLP", "LLM", "data science", "analytics",
    "risk management", "compliance", "regulatory", "governance",
    "project management", "agile", "scrum", "devops", "ci/cd",
    "banking", "payments", "capital markets", "insurance", "wealth",
]
...
def detect_risks(text: str, scope: ScopeResult) -> list[RiskFlag]:
    ...
    if len(scope.deliverables) > 5:
        risks.append(RiskFlag("scope_creep", f"{len(scope.deliverables)} deliverables listed — scope may be too broad", "medium"))
    if "phase 1" in t or "phase one" in t:
        risks.append(RiskFlag("scope_creep", "Multi-phase engagement — scope boundaries need definition", "low"))

    # Regulatory risk
    if len(scope.regulatory_context) > 3:
        risks.append(RiskFlag("regulatory", f"Multiple regulatory frameworks ({len(scope.regulatory_context)}) — compliance overhead", "medium"))

    # Resource risk
    if scope.team_size and scope.team_size > 10:
        risks.append(RiskFlag("resource", f"Large team ({scope.team_size}) — coordination overhead", "medium"))
