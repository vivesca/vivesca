...
class CaseStudy:
    ...
    def to_executive_summary(self) -> str:
        ...
        return (
            f"{self.title}: {self.challenge} "
            f"Our approach: {self.approach} "
...
def _extract_metrics(text: str) -> list[str]:
    ...
    for match in re.finditer(r"([\$\£\€HK]*\d+(?:,\d{3})*(?:\.\d+)?\s*[kKmMbB]?\s*[^.]{5,40})", text):
...
def package_use_case(file_path: str | Path, anonymise: bool = False) -> CaseStudy:
    ...
    challenge = _extract_section(text, "Challenge") or _extract_section(text, "Problem") or _extract_section(text, "Context")
    approach = _extract_section(text, "Approach") or _extract_section(text, "Solution") or _extract_section(text, "Method")
    result = _extract_section(text, "Result") or _extract_section(text, "Outcome") or _extract_section(text, "Impact")
