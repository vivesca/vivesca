...
If a requirements doc exists in `~/metabolon/loci/brainstorms/` matching the topic, read it and use it as the origin. Reference it with `origin:` in frontmatter. Don't re-ask questions the transcription already answered.
...
- **Standard/Deep:** Read relevant source files, check `~/metabolon/loci/solutions/` for learnings, note patterns to follow. Check for system-wide impacts: what callbacks, middleware, or observers fire when this code runs? What breaks if this fails halfway?
...
1. **Search `~/metabolon/loci/solutions/`** for entries matching the domain — prior learnings encode constraints that were discovered the hard way
2. **Search `~/metabolon/loci/plans/`** for similar past plans — what did they require that this spec doesn't mention?
...
- Prior: `~/metabolon/loci/solutions/auth-token-rotation.md` — token refresh needs retry logic (not in spec)
...
date: YYYY-MM-DD
topic: <kebab-case>
origin: ~/metabolon/loci/brainstorms/YYYY-MM-DD-<topic>.md  # if one exists
---
...
Write the plan to `~/metabolon/loci/plans/YYYY-MM-DD-<topic>-plan.md`. Confirm the path.
