...
Read `~/code/epigenome/chromatin/NOW.md` — look for anything unresolved that might race tonight.
...
2. **If the task is ad-hoc** (named during brain dump, no kinesin entry) → append to `~/code/epigenome/chromatin/agent-queue.yaml`:
...
- `~/code/epigenome/chromatin/agent-queue.yaml` — fallback queue for ad-hoc tasks (Step 5)
