...
- `--vault` auto-saves to `~/code/epigenome/chromatin/Councils/` with Obsidian Sync. Never `--output /tmp/` — wiped on reboot.
...
**Step 3: Parse and present** — read `[DECISION]` line, read vault file in `~/code/epigenome/chromatin/Councils/`, synthesize: decision + key reasoning + dissents + cost. Never dump raw transcript into context.
