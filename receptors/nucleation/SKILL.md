...
Orchestrate here, execute elsewhere. Full reference: `~/vivesca/loci/solutions/rector-reference.md`
...
- **Agent Teams** (TeamCreate): when true coordination needed (shared API design, exploratory refactor, unknown-scope bugs) — see `~/vivesca/loci/solutions/rector-reference.md` for decomposition, topology, and parallelism heuristics
...
- If new CLI built: add to `~/vivesca/proteome.md`. If replacing old tool: run `~/vivesca/loci/solutions/patterns/tool-replacement-checklist.md`.
- Skip for: personal tools on main, single-commit changes

### 5. Companion skill + GitHub repo

Create `~/vivesca/receptors/<name>/SKILL.md` in the same session — gotchas are freshest now.

```bash
mkdir -p ~/vivesca/receptors/<name>
# write SKILL.md, then:
...
`/compound-engineering:ce-compound` — captures learnings in `~/vivesca/loci/solutions/`.
...
**Default: Python.** Rust only when: CPU-bound, startup speed, extending existing Rust tool. Capco/client = always Python. Full: `~/vivesca/loci/solutions/rust-vs-python-heuristic.md`.

## Troubleshooting

See `~/vivesca/loci/solutions/rector-reference.md` for: routing tables, launch commands, context packaging checklist, systematic debugging, troubleshooting quick reference, parallel delegation pipeline details, Agent Teams patterns, post-delegate checklist.

Also: `~/vivesca/loci/solutions/delegation-reference.md` for tool-specific gotchas.
