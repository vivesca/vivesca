...
3. **Data** — `~/notes/GARP RAI Quiz Tracker.md` (human-readable stats + history) and `~/notes/.garp-fsrs-state.json` (FSRS scheduling state). Source material in `~/notes/GARP RAI Module {1-5} - Raw Content.md`.
...
**If CLI fails**, fall back: read `~/notes/GARP RAI Quiz Tracker.md` + `~/notes/.garp-fsrs-state.json` manually.
...
**IMPORTANT: All source files must be read using absolute paths (`/Users/terry/notes/...`), never `~/notes/...`. Bash grep fails silently on paths with spaces — always use the Read or Grep tool with the full path.**

1. `/Users/terry/notes/GARP RAI Definition Drills.md` — **mandatory when session plan shows `[drill]` tag.** Contains comparison tables, "exam trap" callouts, and "key distinction" sections. Read the relevant section and specifically test the documented trap pattern. **If a drill entry exists and you skip it, the question quality is compromised.**
2. `/Users/terry/notes/GARP RAI Review Questions.md` — official GARP questions closest to exam voice. Search for the relevant module/chapter section and use or adapt these questions before inventing new ones.
3. `/Users/terry/notes/GARP RAI Fairness Scenarios Drill.md` — **mandatory for M3-demographic-parity, M3-predictive-rate-parity, M3-equal-opportunity, M3-equalized-odds, M3-individual-fairness** (the 5 fairness measure topics). 10 curated scenarios with keyed answers. Use these scenarios (or variations) instead of inventing from scratch.
4. `/Users/terry/notes/GARP RAI Practice Exam.md` — exam-style questions (save for mock/ramp phase)
...
**Before generating:** Read `~/notes/GARP RAI Trap Patterns.md` and grep for the topic. If a pattern entry exists, the question MUST target that exact failure mode — not a generic question on the topic. This step is mandatory, not optional.
...
- **Scenario-based where possible.** Model stems after the Practice Exam (`/Users/terry/notes/GARP RAI Practice Exam.md`): "A bank uses...", "An analyst is...", "A company is...".
...
- Check `~/notes/GARP RAI Trap Patterns.md` — name the pattern if it matches
...
- If any new mistake patterns emerged, append to `~/notes/GARP RAI Trap Patterns.md`
...
- **Quiz tracker:** `~/notes/GARP RAI Quiz Tracker.md`
- **FSRS state:** `~/notes/.garp-fsrs-state.json`
- **Review questions:** `~/notes/GARP RAI Review Questions.md` — official GARP questions, first choice for sourcing
- **Fairness scenarios:** `~/notes/GARP RAI Fairness Scenarios Drill.md` — 10 curated scenarios with keyed answers
- **Definition drills:** `~/notes/GARP RAI Definition Drills.md` — comparison tables + exam traps for weak topics
- **Practice exam:** `~/notes/GARP RAI Practice Exam.md`
- **Trap patterns:** `~/notes/GARP RAI Trap Patterns.md`
- **Glossary:** `~/notes/GARP RAI Glossary.md` — acronyms (useful for rapid-fire)
- **Raw content:** `~/notes/GARP RAI Module {1-5} - Raw Content.md` — verbatim from GARP portal, verified complete Mar 2026
- **GARP portal:** `https://garplearning.benchprep.com/app/rai26` — login flow + navigation gotchas: `~/docs/solutions/garp-portal-navigation.md`
