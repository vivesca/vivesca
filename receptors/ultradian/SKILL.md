...
→ Surface today's acta (daily AI brief). Check: `ls ~/notes/Thalamus/Daily/$(date -v-1d +%Y-%m-%d).md 2>/dev/null || ls ~/notes/Thalamus/Daily/$(date +%Y-%m-%d).md 2>/dev/null`. Read and present key items grouped by lens. Flag any 🚨 READ ORIGINAL with links. If first meeting is soon, also surface one-line prep for it. This is the primary reading window — give the full brief, not a teaser.
